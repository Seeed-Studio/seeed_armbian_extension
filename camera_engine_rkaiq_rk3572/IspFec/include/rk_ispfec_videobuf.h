...
static inline int rk_ispfec_api_vb_sync_start(int fd, int write)
{
    struct dma_buf_sync sync = {
        .flags =  (uint64_t)(DMA_BUF_SYNC_START | (write ? DMA_BUF_SYNC_WRITE : DMA_BUF_SYNC_READ))
    };
...
{
    struct dma_buf_sync sync = {
        .flags =  (uint64_t)(DMA_BUF_SYNC_END | (write ? DMA_BUF_SYNC_WRITE : DMA_BUF_SYNC_READ))
    };
    return ioctl(fd, DMA_BUF_IOCTL_SYNC, &sync);
}
