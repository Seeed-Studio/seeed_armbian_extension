...
#define RK_ISP_FEC_API_VERSION      FEC_VERSION_ENCODE(4, 1, 2)
...
enum rk_ispfec_rotation_view_type {
    RK_ISPFEC_ROTATION_VIEW_TYPE_ALL = 0x0,
    RK_ISPFEC_ROTATION_VIEW_TYPE_TYPICAL = 0x1,
    RK_ISPFEC_ROTATION_VIEW_TYPE_INSIDE = 0x2,
    RK_ISPFEC_ROTATION_VIEW_TYPE_MANUAL = 0x3
};

typedef struct rk_ispfec_rotation_info_s{
    enum rk_ispfec_rotation_view_type view_type;
    float rotation_angle;
}rk_ispfec_rotation_info_t;
...
typedef struct gen_mesh_online_info_s {
    ...
    int correct_level;
    rk_ispfec_rotation_info_t rotation_info;
...
/**
 * @brief FEC working mode enumeration
 *
 * RK_ISPFEC_DISTORTION_CORRECTION_MODE - use fec to correct distortion
 * RK_ISPFEC_ROTATION_MODE              - use fec to rotate image
 */
enum rk_ispfec_working_mode {
    RK_ISPFEC_DISTORTION_CORRECTION_MODE = 0,
    RK_ISPFEC_ROTATION_MODE = 1
};
...
typedef struct rk_ispfec_cfg_s {
    ...
    enum rk_ispfec_working_mode working_mode;
#ifdef RKFEC_HW_V20
    int border_mode;
    int cross_buf_mode;
...
/**
 * @brief Get current FEC processing angle
 * @param[in] ctx Context handle created by rk_ispfec_api_init
 * @param[out] rotation_info Pointer to new configuration parameters
 * @return 0 for success, non-zero for error code
 */
int rk_ispfec_api_getRotation(rk_ispfec_ctx_t* ctx, rk_ispfec_rotation_info_t* rotation_info);

/**
 * @brief Update FEC processing angle
 * @param[in] ctx Context handle created by rk_ispfec_api_init
 * @param[in] rotation_info Pointer to new configuration parameters
 * @return 0 for success, non-zero for error code
 */
 ...
 * @param[in] rotation_info rotation_info to configure
 * @param[out] dst_width Output image width
 * @param[out] dst_height Output image height
 * @return 0 for success, non-zero for error code
 */
