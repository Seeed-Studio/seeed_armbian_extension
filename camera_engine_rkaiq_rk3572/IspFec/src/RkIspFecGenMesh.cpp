                         ...
                         int32_t dstWidth, int32_t dstHeight)
{
    ...
    /* 下采样浮点表的相关参数 */
    memset(&mFloatMeshInfoSP, 0, sizeof(mFloatMeshInfoSP));
    ...
    /* 计算下采样浮点表的尺寸参数 */
    mFloatMeshInfoSP.srcW = srcWidth;
    mFloatMeshInfoSP.srcH = srcHeight;
    mFloatMeshInfoSP.dstW = dstWidth;
    mFloatMeshInfoSP.dstH = dstHeight;
    mFloatMeshInfoSP.enMeshType = enMeshType;              /* 硬件类型 */
    mFloatMeshInfoSP.enFecVersion = enFecVersion;          /* 硬件FEC版本 */
    mFloatMeshInfoSP.enMeshStepOpt = enMeshStepOpt;        /* 浮点映射表的采样步长选项 */
    ret = calcFloatMeshSizeSP(&mFloatMeshInfoSP);
    if (-1 == ret) {
        rkfec_err("%s: failed to calcFloatMeshSizeSP", __func__);
        goto exit;
    }

    /* 下采样浮点表的内存申请 */
    mPf32MeshX = std::unique_ptr<float[]>(new float[mFloatMeshInfoSP.u64FloatMeshBufSize]);
    mPf32MeshY = std::unique_ptr<float[]>(new float[mFloatMeshInfoSP.u64FloatMeshBufSize]);
...
int32_t RkIspFecGenMesh::genMesh(float angle)
{
    int32_t ret = -1;
    ...
    rkfec_dbg(3, rkfec_debug, "the angle of mesh to be generated: %f", angle);

    static Profiler prof = {0};
    if (rkfec_debug > 4) rkfec_profiling_start(&prof);

    /* 根据校正等级angle，生成下采样浮点表 */
    ret = genFloatMeshForRotate(&mFloatMeshInfoSP, angle, mPf32MeshX.get(), mPf32MeshY.get());
    if (-1 == ret) {
        rkfec_err("%s: failed to genFloatMeshForRotate", __func__);
        return ret;
    }

    if (rkfec_debug > 4) rkfec_profiling_end(&prof, "genFloatMeshForRotate", 30);
    ...
    if (rkfec_debug > 4) {
        ...
        /* 保存下采样浮点表 */
        saveFloatMeshSP(savePath, &mFloatMeshInfoSP, mPf32MeshX.get(), mPf32MeshY.get(), angle);

        /* 保存FEC定点化表 */
        saveFecMesh(savePath, &mFecInfo, &mFecMeshBuf, angle);
    }
...
int32_t RkIspFecGenMesh::saveFecMesh(const char* savePath, const RKALGO_FEC_INFO_S* pStFecInfo,
                                     const RKALGO_FEC_MESH_S* pStFecMeshBuf, float angle) {
    uint32_t srcW                     = pStFecInfo->srcW;
    uint32_t srcH                     = pStFecInfo->srcH;
    uint32_t dstW                     = pStFecInfo->dstW;
    uint32_t dstH                     = pStFecInfo->dstH;
    ...
    if (RKALGO_FEC_VERSION_1126B == enFecVersion) {
        ...
        char fecMeshPath[1024];
        if (angle >= 0) {
            sprintf(fecMeshPath, "%s/rv1126b_fecMesh_src%dx%d_dst%dx%d_step%dx%d_angle%03f.bin",
                    savePath, srcW, srcH, dstW, dstH, meshStepW, meshStepH, angle);
        } else {
            sprintf(fecMeshPath, "%s/rv1126b_fecMesh_src%dx%d_dst%dx%d_step%dx%d.bin", savePath,
                    srcW, srcH, dstW, dstH, meshStepW, meshStepH);
        }
