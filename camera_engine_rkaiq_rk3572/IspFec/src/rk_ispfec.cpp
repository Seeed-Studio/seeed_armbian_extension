...
_configure(rk_ispfec_ctx_t* ctx, rk_ispfec_cfg_t* cfg)
{
    ...
    if (cfg->mesh_upd_mode == RK_ISPFEC_UPDATE_MESH_ONLINE ||
        ...
        if(cfg->working_mode == RK_ISPFEC_DISTORTION_CORRECTION_MODE )
...
#ifdef RKFEC_HW_V20
            ret = rk_ispfec_api_vb_sync_start(cfg->mesh_info.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(pre-write) failed: %s", strerror(errno));
#else
            ret = rk_ispfec_api_vb_sync_start(cfg->mesh_xint.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(pre-write) mesh_xint failed: %s", strerror(errno));
            ret = rk_ispfec_api_vb_sync_start(cfg->mesh_xfra.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(pre-write) mesh_xfra failed: %s", strerror(errno));
            ret = rk_ispfec_api_vb_sync_start(cfg->mesh_yint.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(pre-write) mesh_yint failed: %s", strerror(errno));
            ret = rk_ispfec_api_vb_sync_start(cfg->mesh_yfra.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(pre-write) mesh_yfra failed: %s", strerror(errno));
#endif

            if (cfg->working_mode == RK_ISPFEC_DISTORTION_CORRECTION_MODE)
                ret = ctx->_fecGenMesh->genMesh(cfg->u.mesh_online.correct_level);
            else
                ret = ctx->_fecGenMesh->genMesh(cfg->u.mesh_online.rotation_info.rotation_angle);
...
#ifdef RKFEC_HW_V20
            ret = rk_ispfec_api_vb_sync_end(cfg->mesh_info.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(post-write) failed: %s", strerror(errno));
#else
            ret = rk_ispfec_api_vb_sync_end(cfg->mesh_xint.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(post-write) mesh_xint failed: %s", strerror(errno));
            ret = rk_ispfec_api_vb_sync_end(cfg->mesh_xfra.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(post-write) mesh_xfra failed: %s", strerror(errno));
            ret = rk_ispfec_api_vb_sync_end(cfg->mesh_yint.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(post-write) mesh_yint failed: %s", strerror(errno));
            ret = rk_ispfec_api_vb_sync_end(cfg->mesh_yfra.dmaFd, 1);
            if (ret) rkfec_err("_configure: DMA sync(post-write) mesh_yfra failed: %s", strerror(errno));
#endif
        }
#endif
...
int rk_ispfec_api_getRotation(rk_ispfec_ctx_t* ctx, rk_ispfec_rotation_info_t* rotation_info) {
    if (!ctx) return -1;
    rk_ispfec_cfg_t cfg = {0};

    if (rk_ispfec_api_get_cfg(ctx, &cfg) == -1) {
        rkfec_err("get angle cfg process error");
        return -1.0f;
    }

    memcpy(rotation_info, &cfg.u.mesh_online.rotation_info, sizeof(rk_ispfec_rotation_info_t));

    return 0;
}

int rk_ispfec_api_setRotation(rk_ispfec_ctx_t* ctx, rk_ispfec_rotation_info_t* rotation_info) {
    if (!ctx) return -1;
    rk_ispfec_cfg_t cfg = {0};

    rk_ispfec_api_get_cfg(ctx, &cfg);

    if (cfg.u.mesh_online.rotation_info.view_type == RK_ISPFEC_ROTATION_VIEW_TYPE_MANUAL) {
        rkfec_err("rotation view type is set to manual, can't set rotation angle");
        return -1;
    }
        
    memcpy(&cfg.u.mesh_online.rotation_info, rotation_info, sizeof(rk_ispfec_rotation_info_t));

    return rk_ispfec_api_set_cfg(ctx, &cfg);
}
...
// 检查点 (a, b) 是否在旋转后的图像内
bool check_inside_rotated(double a, double b, double halfW, double halfH,
    ...
    // x = a*cosα + b*sinα
    // y = -a*sinα + b*cosα
    double x = a * cos_alpha + b * sin_alpha;
    double y = -a * sin_alpha + b * cos_alpha;
    
    return (fabs(x) <= halfW + EPS && fabs(y) <= halfH + EPS);
}

// 主函数：计算旋转后图像内的最大内接矩形（轴对齐，中心在原点）
int getRotation_Maxview(int src_width, int src_height,
                                  float angle, int* dst_width, int* dst_height) {
    if (src_width <= 0 || src_height <= 0 ||
        dst_width == nullptr || dst_height == nullptr) {
        return -1;
    }

    double alpha = static_cast<double>(angle);
    
    // 将角度标准化到[0, 360)
    alpha = fmod(alpha, 360.0);
    if (alpha < 0) alpha += 360.0;
    
    double halfW = src_width / 2.0;
    double halfH = src_height / 2.0;
    
    // 计算下界 L = H/√2
    double L = src_height / sqrt(2.0);
    double half_L = L / 2.0;
    
    // 将角度转换为弧度
    double rad = alpha * PI / 180.0;
    double cos_alpha = cos(rad);
    double sin_alpha = sin(rad);
    
    // 处理特殊情况
    if (alpha < EPS || fabs(alpha - 180.0) < EPS || fabs(alpha - 360.0) < EPS) {
        // 0°, 180°, 360°: 旋转后图像与原图相同
        double a = halfW;
        double b = halfH;
        if (2*a < L - EPS || 2*b < L - EPS) {
            *dst_width = static_cast<int>(L + 0.5);
            *dst_height = static_cast<int>(L + 0.5);
        } else {
            *dst_width = static_cast<int>(2*a + 0.5);
            *dst_height = static_cast<int>(2*b + 0.5);
        }
        *dst_width = static_cast<int>(floor(*dst_width / 16) * 16);
        *dst_height = static_cast<int>(floor(*dst_height / 16) * 16);
        return 0;
    } else if (fabs(alpha - 90.0) < EPS || fabs(alpha - 270.0) < EPS) {
        // 90°, 270°: 旋转后图像宽高互换
        double a = halfH;  // 旋转后宽度变为原高度
        double b = halfW;  // 旋转后高度变为原宽度
        if (2*a < L - EPS || 2*b < L - EPS) {
            *dst_width = static_cast<int>(L + 0.5);
            *dst_height = static_cast<int>(L + 0.5);
        } else {
    ...
    // 我们需要在旋转后的图像内找到最大内接矩形
    // 旋转后图像边界条件: 点(x,y)满足 |x*cosα + y*sinα| ≤ W/2 且 |-x*sinα + y*cosα| ≤ H/2
    // 内接矩形顶点(±a, ±b)必须满足这个条件
    
    // 由于对称性，只需要检查(a,b)和(a,-b)两个点
    // 约束条件:
    // 1. |a*cosα + b*sinα| ≤ W/2
    // 2. |-a*sinα + b*cosα| ≤ H/2
    // 3. |a*cosα - b*sinα| ≤ W/2
    ...
    // 1. a*cosα + b*sinα ≤ W/2
    // 2. a*cosα - b*sinα ≤ W/2
    // 3. a*cosα + b*sinα ≥ -W/2 (自动满足，因为左边为正)
    // 4. a*cosα - b*sinα ≥ -W/2 (可能不满足，需要考虑)
    // 5. -a*sinα + b*cosα ≤ H/2
    // 6. -a*sinα - b*cosα ≤ H/2
    // 7. -a*sinα + b*cosα ≥ -H/2 (可能不满足，需要考虑)
    // 8. -a*sinα - b*cosα ≥ -H/2 (自动满足，因为右边为正时左边为负)
    ...
    // 由于a,b>0，且α在(0,180)区间，可以进一步简化
    
    // 候选解1: 固定a求最大b
    // 从约束1: b ≤ (W/2 - a*cosα)/sinα
    // 从约束3: b ≤ (H/2 + a*sinα)/cosα
    // 从约束2: b ≥ (a*cosα - W/2)/sinα (当sinα>0) 或 b ≤ ... (当sinα<0)
    // 从约束4: b ≥ (-H/2 - a*sinα)/cosα (当cosα>0) 或 b ≤ ... (当cosα<0)
    
    // 由于α可能在0-360度，我们需要分情况讨论
    // 但更简单的方法是: 在a的可行范围内搜索，对每个a求满足所有约束的最大b
    
    // 使用二分搜索法
    double a_min = half_L;
    double a_max = halfW;  // a不能超过原图半宽
    int steps = 100;  // 搜索步数
    
    for (int i = 0; i <= steps; i++) {
        double a = a_min + (a_max - a_min) * i / steps;
        
        if (a < half_L - EPS) continue;
        
        // 对给定的a，计算b的上界和下界
        double b_upper1 = (halfW - a * fabs(cos_alpha)) / fabs(sin_alpha);
        double b_upper2 = (halfH - a * fabs(sin_alpha)) / fabs(cos_alpha);
        double b_upper = std::min(b_upper1, b_upper2);
        
        double b_lower1 = 0;
        if (fabs(sin_alpha) > EPS) {
            b_lower1 = (-halfW + a * fabs(cos_alpha)) / fabs(sin_alpha);
        }
        double b_lower2 = 0;
        if (fabs(cos_alpha) > EPS) {
            b_lower2 = (-halfH + a * fabs(sin_alpha)) / fabs(cos_alpha);
        }
        double b_lower = std::max(b_lower1, b_lower2);
        b_lower = std::max(b_lower, half_L);
        
        if (b_upper < b_lower - EPS) continue;
        
        // 尝试b = b_upper, b = b_lower, 以及两者之间的点
        std::vector<double> b_vals;
        b_vals.push_back(b_lower);
        b_vals.push_back(b_upper);
        for (int j = 1; j < 5; j++) {
            double b = b_lower + (b_upper - b_lower) * j / 5.0;
            b_vals.push_back(b);
        }
        
        for (double b : b_vals) {
            if (b < half_L - EPS) continue;
            ...
            for (int s1 = -1; s1 <= 1; s1 += 2) {
                for (int s2 = -1; s2 <= 1; s2 += 2) {
                    double x = a * s1;
                    double y = b * s2;
                    if (!check_inside_rotated(x, y, halfW, halfH, cos_alpha, sin_alpha)) {
                        ok = false;
                        break;
                    }
                }
    ...
    // 同样，固定b搜索a
    double b_min = half_L;
    double b_max = halfH;  // b不能超过原图半高
    
    for (int i = 0; i <= steps; i++) {
        double b = b_min + (b_max - b_min) * i / steps;
        
        if (b < half_L - EPS) continue;
        
        // 对给定的b，计算a的上界和下界
        double a_upper1 = (halfW - b * fabs(sin_alpha)) / fabs(cos_alpha);
        double a_upper2 = (halfH - b * fabs(cos_alpha)) / fabs(sin_alpha);
        double a_upper = std::min(a_upper1, a_upper2);
        
        double a_lower1 = 0;
        if (fabs(cos_alpha) > EPS) {
            a_lower1 = (-halfW + b * fabs(sin_alpha)) / fabs(cos_alpha);
        }
        double a_lower2 = 0;
        if (fabs(sin_alpha) > EPS) {
            a_lower2 = (-halfH + b * fabs(cos_alpha)) / fabs(sin_alpha);
        }
        double a_lower = std::max(a_lower1, a_lower2);
        a_lower = std::max(a_lower, half_L);
        
        if (a_upper < a_lower - EPS) continue;
        
        // 尝试a = a_upper, a = a_lower, 以及两者之间的点
        std::vector<double> a_vals;
        a_vals.push_back(a_lower);
        a_vals.push_back(a_upper);
        for (int j = 1; j < 5; j++) {
            double a = a_lower + (a_upper - a_lower) * j / 5.0;
            a_vals.push_back(a);
        }
    ...
    // 转换为整型宽高（四舍五入）
    *dst_width = static_cast<int>(2 * best.a + 0.5);
    *dst_height = static_cast<int>(2 * best.b + 0.5);
    
    // 确保宽高不小于下界 L
    if (*dst_width < L - 1.0 || *dst_height < L - 1.0) {
        *dst_width = static_cast<int>(L + 0.5);
        *dst_height = static_cast<int>(L + 0.5);
    }
...
int rk_ispfec_api_setRotation_view(int src_width, int src_height, rk_ispfec_rotation_info_t rotation_info, int* dst_width, int* dst_height)
{
    if (dst_width == NULL || dst_height == NULL || src_width <= 0 || src_height <= 0) {
        return -1; // 参数错误
    }

    int tmp;
    switch (rotation_info.view_type)
    {
    case RK_ISPFEC_ROTATION_VIEW_TYPE_ALL:
        tmp = ceil(sqrt(src_width * src_width + src_height * src_height) / 16) * 16;
        *dst_width = tmp;
        *dst_height = tmp;
        return 0;
    case RK_ISPFEC_ROTATION_VIEW_TYPE_TYPICAL:
        *dst_width = src_width;
        *dst_height = src_height;
        return 0;
    case RK_ISPFEC_ROTATION_VIEW_TYPE_INSIDE:
        tmp = static_cast<int>(floor(((float)src_height / 1.4142) / 16) * 16);
        *dst_width = tmp;
        *dst_height = tmp;
        return 0;
    default:
    ...
    return getRotation_Maxview(src_width, src_height, rotation_info.rotation_angle, dst_width, dst_height);
}
    ...
    switch (config->mode) {
...
#ifdef RKFEC_HW_V20
                ret = rk_ispfec_api_vb_sync_start(ctx->_cfg.mesh_info.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(pre-write) failed: %s", strerror(errno));
#else
                ret = rk_ispfec_api_vb_sync_start(ctx->_cfg.mesh_xint.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(pre-write) mesh_xint failed: %s", strerror(errno));
                ret = rk_ispfec_api_vb_sync_start(ctx->_cfg.mesh_xfra.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(pre-write) mesh_xfra failed: %s", strerror(errno));
                ret = rk_ispfec_api_vb_sync_start(ctx->_cfg.mesh_yint.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(pre-write) mesh_yint failed: %s", strerror(errno));
                ret = rk_ispfec_api_vb_sync_start(ctx->_cfg.mesh_yfra.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(pre-write) mesh_yfra failed: %s", strerror(errno));
#endif

                if (ctx->_cfg.working_mode == RK_ISPFEC_DISTORTION_CORRECTION_MODE)
                    ret = ctx->_fecGenMesh->genMesh(config->params.online_cfg.correct_level);
                else
                    ret = ctx->_fecGenMesh->genMesh(config->params.online_cfg.rotation_info.rotation_angle);
...
#ifdef RKFEC_HW_V20
                ret = rk_ispfec_api_vb_sync_end(ctx->_cfg.mesh_info.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(post-write) failed: %s", strerror(errno));
#else
                ret = rk_ispfec_api_vb_sync_end(ctx->_cfg.mesh_xint.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(post-write) mesh_xint failed: %s", strerror(errno));
                ret = rk_ispfec_api_vb_sync_end(ctx->_cfg.mesh_xfra.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(post-write) mesh_xfra failed: %s", strerror(errno));
                ret = rk_ispfec_api_vb_sync_end(ctx->_cfg.mesh_yint.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(post-write) mesh_yint failed: %s", strerror(errno));
                ret = rk_ispfec_api_vb_sync_end(ctx->_cfg.mesh_yfra.dmaFd, 1);
                if (ret) rkfec_err("set_mesh_config: DMA sync(post-write) mesh_yfra failed: %s", strerror(errno));
#endif
        ...
        case RK_ISPFEC_UPDATE_MESH_FROM_FILE: {
            ...
            auto process_component = [&](const char* full_path, mesh_info_t& mesh_info) {
                ...
                int sync_ret = rk_ispfec_api_vb_sync_start(mesh_info.dmaFd, 1);
                if (sync_ret) rkfec_err("set_mesh_config: DMA sync(pre-write) failed: %s", strerror(errno));
                int load_ret = _load_mesh(full_path, mesh_info.vir_addr, mesh_info.size);
                sync_ret = rk_ispfec_api_vb_sync_end(mesh_info.dmaFd, 1);
                if (sync_ret) rkfec_err("set_mesh_config: DMA sync(post-write) failed: %s", strerror(errno));
                return load_ret;
            };
            ...
            auto process_component = [&](mesh_info_t& dst, const rk_ispfec_buffer_info_t& src) {
                ...
                if (src.need_copy) {
                    if (dst.vir_addr && src.vir_addr) {
                        ...
                        int sync_ret = rk_ispfec_api_vb_sync_start(dst.dmaFd, 1);
                        if (sync_ret) rkfec_err("set_mesh_config: DMA sync(pre-write) failed: %s", strerror(errno));
                        memcpy(dst.vir_addr, src.vir_addr, copy_size);
                        sync_ret = rk_ispfec_api_vb_sync_end(dst.dmaFd, 1);
                        if (sync_ret) rkfec_err("set_mesh_config: DMA sync(post-write) failed: %s", strerror(errno));
...
int rk_ispfec_api_load_online_config_from_ini(const char* ini_path,
    ...
    info_out->rotation_info.view_type = (enum rk_ispfec_rotation_view_type)rk_ispfec_param_get_int("Config:sw_ldcT_rotation_view", 0);
    info_out->rotation_info.rotation_angle = (float)rk_ispfec_param_get_double("Config:sw_ldcT_rotation_angle", 0);
    info_out->style =
