...
static void calcHdrIso(AiqCamGroupManager_t* pCamGrpMan, RKAiqAecExpInfo_t* pCurExp, rk_aiq_singlecam_3a_result_t *singleCam3aRes)
{
    ...
    if (working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
        ...
        arIspdgain = pCurExp->LinearExp.exp_real_params.isp_dgain;
        if (arAGain < 1.0) {
            arAGain = 1.0;
        }
        if (arDGain < 1.0) {
        ...
        singleCam3aRes->hdrIso = arAGain * arDGain * arIspdgain * 50;
        singleCam3aRes->hdrIsoWithoutIspdgain = arAGain * arDGain * 50;
    } else {
        int i = 0;
        ...
        arIspdgain = pCurExp->HdrExp[i].exp_real_params.isp_dgain;
