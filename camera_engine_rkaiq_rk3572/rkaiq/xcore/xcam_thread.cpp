...
bool Thread::start ()
{
    ...
    if (pthread_create(&_thread_id, &attr, (void* (*)(void*))thread_func, this) != 0) {
        // 如果设置了优先级且创建失败，尝试使用默认设置重新创建
        if (use_priority) {
            XCAM_LOG_WARNING("Thread(%s) create with priority failed, try default settings",
                             XCAM_STR(_name));
            // 销毁之前的属性
            ...
            if (pthread_create(&_thread_id, &attr, (void* (*)(void*))thread_func, this) != 0) {
                pthread_attr_destroy(&attr);
                XCAM_LOG_ERROR("Thread(%s) create with default settings also fail !",
                               XCAM_STR(_name));
                return false;
            }
        } else {
