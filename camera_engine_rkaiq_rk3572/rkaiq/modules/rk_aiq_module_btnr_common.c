...
int bayertnr_logtrans(uint32_t tmpfix, btnr_trans_params_t *pTransPrarms)
{
    ...
    {
#ifdef ISP_HW_V351s
        int tmpfix1 = (int)tmpfix;
        x8 = MIN((ABS(tmpfix1 - pTransPrarms->transf_mode_OB) + pTransPrarms->transf_mode_offset), pTransPrarms->transf_data_max_limit);
        long long sign = (tmpfix1 - pTransPrarms->transf_mode_OB) < 0 ? -1 : 1;
#else
        x8 = MIN((tmpfix + pTransPrarms->transf_mode_offset), pTransPrarms->transf_data_max_limit);
#endif
        // find highest bit
...
#ifdef ISP_HW_V351s
        fx = (sign<0) ? (-fx) : (fx);
        fx = fx + pTransPrarms->transf_mode_offset1;
#endif
    }

#ifdef ISP_HW_V351s 
	return (int)CLIP(fx, 0, 4095);  
#else
	return (int)fx;
#endif
}
...
#elif defined(RKAIQ_HAVE_BAYERTNR_V42) || defined(RKAIQ_HAVE_BAYERTNR_V51) 
    {
#ifdef ISP_HW_V351s
        struct rkisp351s_stat_buffer* stats = stats_buffer;
#else
