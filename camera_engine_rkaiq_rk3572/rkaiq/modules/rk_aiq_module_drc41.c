...
#elif ISP_HW_V351s
    struct isp351s_drc_cfg* phwcfg = &isp_params->isp_cfg->others.drc_cfg;
#endif
...
#if ISP_HW_V351s
    float log_gain   = log(drc_gain / preDGain_drc) / log(2.0f);
    float log_gain_m = 2.0f + log_gain - floor(log_gain);
#endif
    if (pdyn->preProc.sw_drcT_toneCurve_mode == drc_cfgCurveCtrlCoeff_mode) {
...
#if ISP_HW_V351s
            float drc_gain_scale = pow(2.0f, log_gain_m) / drc_gain;
            luma2ToneGain_val[i] = luma2ToneGain_val[i] * drc_gain_scale;
#endif
        }
    } else if (pdyn->preProc.sw_drcT_toneCurve_mode == drc_cfgCurveDirect_mode) {
        for (int i = 0; i < 17; i++) {
...
#if ISP_HW_V351s
    phwcfg->gain_shift_bit = 2.0f - floor(log_gain);
