                             ...
                             common_cvt_info_t* cvtinfo, blc_cvt_info_t* pBlcInfo, rk_aiq_wb_gain_v32_t* awb_gain_final)
{
...
#else
    blc_calib_dyn_t *pCalibDyn = &((blc_param_t*)attr)->calibDyn;
	uint16_t autoOB_offset = CLIP(pCalibDyn->obSigma.hw_blcC_obSigmaR_val, 0, 511);
#endif
    ...
    float totalGain   = 1;
    if (cvtinfo->ae_exp) {
        totalGain = cvtinfo->ae_exp->LinearExp.exp_real_params.analog_gain *
...
#endif
#ifdef ISP_HW_V351s
    phwcfg->fixed_val.r = (short)pCalibDyn->obMean.hw_blcC_obMeanR_val + pdyn->obcPreTnr.hw_blcC_obR_offset;
    phwcfg->fixed_val.gr = (short)pCalibDyn->obMean.hw_blcC_obMeanGr_val + pdyn->obcPreTnr.hw_blcC_obGr_offset;
    phwcfg->fixed_val.gb = (short)pCalibDyn->obMean.hw_blcC_obMeanGb_val + pdyn->obcPreTnr.hw_blcC_obGb_offset;
    phwcfg->fixed_val.b  = (short)pCalibDyn->obMean.hw_blcC_obMeanB_val + pdyn->obcPreTnr.hw_blcC_obB_offset;
#endif
