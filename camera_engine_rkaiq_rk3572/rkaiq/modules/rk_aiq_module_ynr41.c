...
void rk_aiq_ynr41_params_cvt(void* attr, isp_params_t* isp_params, common_cvt_info_t *cvtinfo)
{
    ...
    struct isp351s_ynr_cfg *pCfg = &isp_params->isp_cfg->others.ynr_cfg;
...
#if defined(ISP_HW_V351s)
    // REG: HI_SIGMA_LIMIT
    pCfg->hi_spnr_filt_noise_limit_en = pdyn->hiNr.epf.hw_ynrT_noiseLimit_en;
    pCfg->hi_spnr_filt_noise_min_limit_scale = MIN((int)(pdyn->hiNr.epf.hw_ynrT_noiseMinLimit_scale * (1 << 6)), 1023);
    pCfg->hi_spnr_filt_noise_max_limit_scale = MIN((int)(pdyn->hiNr.epf.hw_ynrT_noiseMaxLimit_scale * (1 << 6)), 1023);
    pCfg->hi_spnr_filt_diff_offset_scale = MIN((int)(pdyn->hiNr.epf.hw_ynrT_diffOffset_scale * (1 << 7)), 1023);
    // REG: MI_EDGE_ADJUST
    pCfg->mi_spnr_filt_edge_adjust_en = pdyn->midLoNr.mf.hw_ynrT_edgeAdjust_en;
    pCfg->mi_spnr_filt_edge_scale = MIN((int)(pdyn->midLoNr.mf.hw_ynrT_edgeAdjust_scale * 16), 16);
    pCfg->mi_spnr_filt_edge_offset = MIN((int)(pdyn->midLoNr.mf.hw_ynrT_edgeAdjust_offset * (1 << 10)), 1023);
    // REG: MI_TEXT_ADJUST
    pCfg->mi_spnr_tex_adjust_en = pdyn->midLoNr.mf.hw_ynrT_texAdjust_en;
    pCfg->mi_spnr_tex_scale = MIN((int)(pdyn->midLoNr.mf.hw_ynrT_texAdjust_scale * 32), 255);
    pCfg->mi_spnr_tex_min_weight = MIN((int)(pdyn->midLoNr.mf.hw_ynrT_texAdjust_minWgt * (1 << 10)), 1024);
    pCfg->mi_spnr_tex_thred = MIN((int)(pdyn->midLoNr.mf.hw_ynrT_texAdjust_thred * (1 << 10)), 1023);
#endif
