...
void rk_aiq_dm25_params_cvt(void* attr, isp_params_t* isp_params, common_cvt_info_t *cvtinfo)
{
    ...
    struct isp351s_debayer_cfg *phwcfg = &isp_params->isp_cfg->others.debayer_cfg;
...
#if ISP_HW_V351s
    phwcfg->g_interp_global_sharp_strg_en = pdyn->gInterp.hw_dmT_gInterpGlbSharpStrg_en;
#endif
...
#if ISP_HW_V351s
	/* C_FILTER_MED_GAUS */
    phwcfg->med_wgt_lpf_coeff0 = psta->cFlt.hw_dmT_medWgtLpf_coeff[0];
    phwcfg->med_wgt_lpf_coeff1 = psta->cFlt.hw_dmT_medWgtLpf_coeff[1];
    phwcfg->med_wgt_lpf_coeff2 = psta->cFlt.hw_dmT_medWgtLpf_coeff[2];
#endif
...
#if ISP_HW_V351s
	/* C_FILTER_MED_THRED */
    for (int i = 0; i < ISP351S_DEBAYER_MED_NUM; i++) {
        phwcfg->cnr_med_flt_wgt_min_thred[i] = pdyn->cFlt.hw_dmT_cnrMedFltWgt_minThred[i];
    }
	/* C_FILTER_MED_0 */
	phwcfg->cnr_med_flt_wgt_slope = ROUND_F(pdyn->cFlt.hw_dmT_cnrMedFltWgt_slope * (1 << RK_DM33_FIX_BIT_MED_WGT));
	phwcfg->cnr_med_flt_range_scale = ROUND_F(pdyn->cFlt.hw_dmT_cnrMedFltRange_scale * (1 << RK_DM33_FIX_BIT_MED_FLT_RANGE_SCALE));
	/* C_FILTER_MED_1 */
	phwcfg->cnr_med_flt_index_min = pdyn->cFlt.hw_dmT_cnrMedFltIndex_min;
	phwcfg->cnr_med_flt_index_max = pdyn->cFlt.hw_dmT_cnrMedFltIndex_max;
#endif
    /* C_FILTER_BF */
...
#if ISP_HW_V351s
	/* C_FILTER_EDGE_1 */
    phwcfg->cnr_edge_norm_bit = ROUND_F(pdyn->cFlt.hw_dmT_cnrEdgeNorm_bit);
    phwcfg->cnr_edge_norm_bit = CLIP(phwcfg->cnr_edge_norm_bit, 8, 12);
#endif
    return;
}
