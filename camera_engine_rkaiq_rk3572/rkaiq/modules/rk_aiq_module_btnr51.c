...
void rk_aiq_btnr51_params_logtrans(struct isp351s_bay3d_cfg *pCfg)
{
...
void rk_aiq_btnr51_params_cvt(void* attr, isp_params_t* isp_params, common_cvt_info_t* cvtinfo, btnr_cvt_info_t* pBtnrInfo, mergeLuma2Wgt_t* pMergeLumaWgt)
{
    ...
    struct isp351s_bay3d_cfg *pCfg = &isp_params->isp_cfg->others.bay3d_cfg;

    btnr_param_t *btnr_param = (btnr_param_t *) attr;
    btnr_params_static_t* psta = &btnr_param->sta;
    btnr_calib_dyn_t *pCalibDyn = &btnr_param->calibDyn;
    ...
    if (framenum == 1) {
        frameiso[0] = cvtinfo->frameIsoWithoutIspdgain[0];
        frameiso[1] = cvtinfo->frameIsoWithoutIspdgain[1];
        frameiso[2] = cvtinfo->frameIsoWithoutIspdgain[2];
    }
    else {
    ...
    int bayertnr_iso_cur = frameiso[cvtinfo->frameNum - 1];
    ...
    // REG: BAY3D_TRANS2
    pCfg->transf_mode_offset1 = 0; //TODO
    pCfg->transf_mode_ob = 0; //TODO
    ...
    if(cvtinfo->isFirstFrame) {
        pBtnrInfo->pre_ob = cvtinfo->blc_res.obcPostTnr.sw_blcT_autoOB_offset;
    }
    if(psta->frmFusion.isoSwitch.sw_btnrT_fusionIso_mode == btnr_limitAdj_mode) {
        int delta_ob = ABS(cvtinfo->blc_res.obcPostTnr.sw_blcT_autoOB_offset - pBtnrInfo->pre_ob);
        if(delta_ob >= psta->frmFusion.isoSwitch.sw_btnrT_limitAdj_deltaOB) {
            tmpf = 1;
        }
        if(delta_ob > 0) {
            LOGD_ANR("iso:%d pre_ob:%d cur_ob:%d delta_ob:%d\n", frameiso[0], pBtnrInfo->pre_ob, cvtinfo->blc_res.obcPostTnr.sw_blcT_autoOB_offset, delta_ob);
        }
    }
    pBtnrInfo->pre_ob = cvtinfo->blc_res.obcPostTnr.sw_blcT_autoOB_offset;
#endif
    ...
    pTransParams->transf_data_max_limit = pCfg->transf_data_max_limit;
    pTransParams->itransf_mode_offset = pCfg->itransf_mode_offset;
    pTransParams->transf_mode_OB = pCfg->transf_mode_ob;
    pTransParams->transf_mode_offset1 = pCfg->transf_mode_offset1;
    ...
    if(pTransParams->isTransfBypass  || opMode == RK_AIQ_OP_MODE_MANUAL) {
        for(i = 0; i < sigbins; i++) {
            pCfg->tnr_luma2sigma_x[i] = pCalibDyn->hw_btnrC_mdSigma_curve.idx[i];
    ...
    if(psta->sigmaEnv.sw_btnrCfg_sigma_mode == btnr_manualSigma_mode || cvtinfo->isFirstFrame || bayertnr_default_noise_curve_use) {
        ...
        if( pTransParams->isTransfBypass || opMode == RK_AIQ_OP_MODE_MANUAL) {
            for(i = 0; i < sigbins; i++) {
                pTransParams->tnr_luma_sigma_y[i] = pCalibDyn->hw_btnrC_mdSigma_curve.val[i];
            }
            ...
            uint16_t* pSigmaX = pCfg->tnr_luma2sigma_x;
            uint16_t* pSigmaY = pTransParams->tnr_luma_sigma_y;
            uint16_t* pCalibX = pCalibDyn->hw_btnrC_mdSigma_curve.idx;
            uint16_t* pLongY = pCalibDyn->hw_btnrC_mdSigma_curve.val;
            uint16_t* pShortLowY = btnr_attrib->stAuto.calibDyn[short_ilow].hw_btnrC_mdSigma_curve.val;
            uint16_t* pShortHighY = btnr_attrib->stAuto.calibDyn[short_ihigh].hw_btnrC_mdSigma_curve.val;
            ...
            uint16_t* pLowY = btnr_attrib->stAuto.calibDyn[ilow].hw_btnrC_mdSigma_curve.val;
            uint16_t* pHighY = btnr_attrib->stAuto.calibDyn[ihigh].hw_btnrC_mdSigma_curve.val;
            uint16_t* pLowX = btnr_attrib->stAuto.calibDyn[ilow].hw_btnrC_mdSigma_curve.idx;
            uint16_t* pHighX = btnr_attrib->stAuto.calibDyn[ihigh].hw_btnrC_mdSigma_curve.idx;
            uint16_t* pSigmaX = pCfg->tnr_luma2sigma_x;
            uint16_t* pSigmaY = pTransParams->tnr_luma_sigma_y;
    ...
    if(!auto_sig_curve_spnruse || cvtinfo->isFirstFrame) {
        if(pTransParams->isTransfBypass || opMode == RK_AIQ_OP_MODE_MANUAL) {
            for(i = 0; i < spnrsigbins; i++) {
                pCfg->pre_spnr_luma2sigma_y[i] = CLIP((int)(pCalibDyn->hw_btnrC_preSpNrSgm_curve.val[i]), 0, max_sig);
                pCfg->pre_spnr_luma2sigma_x[i] = pCalibDyn->hw_btnrC_preSpNrSgm_curve.idx[i];
            }
        } else if(RK_AIQ_HDR_IS_SENSOR_BUILTIN(pBtnrInfo->_working_mode)) {
            ...
            uint16_t* pSigmaX = pCfg->pre_spnr_luma2sigma_x;
            uint16_t* pSigmaY = pCfg->pre_spnr_luma2sigma_y;
            uint16_t* pLongY = pCalibDyn->hw_btnrC_preSpNrSgm_curve.val;
            uint16_t* pCalibX = pCalibDyn->hw_btnrC_preSpNrSgm_curve.idx;
            uint16_t* pShortLowY = btnr_attrib->stAuto.calibDyn[short_ilow].hw_btnrC_preSpNrSgm_curve.val;
            uint16_t* pShortHighY = btnr_attrib->stAuto.calibDyn[short_ihigh].hw_btnrC_preSpNrSgm_curve.val;
            for(i = 0; i < spnrsigbins; i++) {
                shortY[i] = pShortLowY[i] + short_iso_ratio * (pShortHighY[i] - pShortLowY[i]);
            }
            ...
            pre_interp(iso, NULL, 0, &ilow, &ihigh, &iso_ratio);
            // interpolate y in low iso
            uint16_t* pLowY = btnr_attrib->stAuto.calibDyn[ilow].hw_btnrC_preSpNrSgm_curve.val;
            uint16_t* pHighY = btnr_attrib->stAuto.calibDyn[ihigh].hw_btnrC_preSpNrSgm_curve.val;
            uint16_t* pLowX = btnr_attrib->stAuto.calibDyn[ilow].hw_btnrC_preSpNrSgm_curve.idx;
            uint16_t* pHighX = btnr_attrib->stAuto.calibDyn[ihigh].hw_btnrC_preSpNrSgm_curve.idx;
            uint16_t* pSigmaX = pCfg->pre_spnr_luma2sigma_x;
            uint16_t* pSigmaY = pCfg->pre_spnr_luma2sigma_y;
    ...
    if (!pCfg->transf_bypass_en) {
        rk_aiq_btnr51_params_logtrans(pCfg);
    }
...
void rk_aiq_btnr51_l2_params_cvt(void* attr, isp_params_t* isp_params, common_cvt_info_t* cvtinfo, btnr_cvt_info_t* pBtnrInfo, mergeLuma2Wgt_t* pMergeLumaWgt)
{
    ...
    float tmpf;
    int i, j, tmp, tmp0, tmp1, sigbins, halfx;
    struct isp351s_bay3d_cfg *pCfg = &isp_params->isp_cfg->others.bay3d_cfg; //驱动目前没有�?
    btnr_param_t *btnr_param = (btnr_param_t *) attr;
    btnr_params_static_t* psta = &btnr_param->sta;
    btnr_calib_dyn_t* pCalibDyn = &btnr_param->calibDyn;
    btnr_other_dyn_t* pdyn = &btnr_param->spNrDyn;
    ...
    if (pdyn->outFrmFusion.sw_btnrT_outFrmBase_mode == btnr_curBaseOut_mode) {
        ...
        tmp0 = MIN((sqrt((float)1.0 / pdyn->outFrmFusion.noiseBal_curBaseOut.hw_btnrT_hiNoiseBal_strg) * (1 << 10)), (1 << 10));
        tmp1 = MIN((sqrt((float)1.0 / pdyn->outFrmFusion.noiseBal_curBaseOut.hw_btnrT_midNoiseBal_strg) * (1 << 10)), (1 << 10));
        pCfg->out_use_hi_noise_bal_nr_strg = MIN((tmp0 + hi_offset) * hi_scale, 4.0 * (1 << 10));
        pCfg->out_use_md_noise_bal_nr_strg = MIN((tmp1 + md_offset) * md_scale, 4.0 * (1 << 10));
        pCfg->out_use_hi_noise_bal_nr_off = hi_offset;
        pCfg->out_use_md_noise_bal_nr_off = md_offset;
    }
    else {
...
#if 0
void bayertnr_isp39_logtrans_test(struct isp351s_bay3d_cfg *pCfg, btnr_trans_params_t *pTransParams)
{
