...
typedef struct {
    /* M4_GENERIC_DESC(
    M4_ALIAS(setWbGnInSensor_en),
    M4_TYPE(bool),
    M4_DEFAULT(0),
    M4_HIDE_EX(0),
    M4_RO(0),
    M4_ORDER(1),
    M4_NOTES(The enable setting wb gain to sensor when hdr mode is buildIn.\n
    Freq of use: low))  */
    bool setWbGnInSensor_en;
} calibdb_cis_wb_t;
...
typedef struct {
    ...
    // M4_STRUCT_DESC("cis_wb", "normal_ui_style")
