...
/*!
 * \brief set master camera sync list for multi-camera synchronization
 *
 * \param[in] ctx             the context returned by \ref rk_aiq_uapi2_sysctl_init
 * \param[in] master_cam_id   master camera id
 * \param[in] sync_cam_ids    array of camera ids to be synchronized with master
 * \param[in] sync_cam_num    number of cameras in sync_cam_ids array
 * \return return 0 if success
 *
 * \code
 * // Example: set camera 0 as master, sync with cameras 1 and 2
 * uint32_t sync_cam_ids[] = {1, 2};
 * rk_aiq_uapi2_setMasterCamSyncList(ctx, 0, sync_cam_ids, 2);
 * \endcode
 */
...
/*!
 * \brief trigger capture operation
 * Trigger a capture sequence for still image or burst capture.
 * In preview-capture mode, this switches from preview (SCL channels)
 * to capture (FULL channels) for high-quality image acquisition.
 *
 * \param[in] ctx             context returned by \ref rk_aiq_uapi2_sysctl_init
 * \param[in] trigger         capture trigger configuration containing frame count
 * \return return XCAM_RETURN_NO_ERROR if success
 */
