...
#define MAX_AWB_TIME_SHARE_FRAME_NUM 4
...
typedef enum rk_aiq_uapiV2_awb_wrtIntvel_mode_s{
    AWB_WRITING_MODE_CURRENT = 0,
    AWB_WRITING_MODE_INTERVAL = 1,
}rk_aiq_uapiV2_awb_wrtIntvel_mode_e;

typedef struct rk_aiq_uapiV2_awb_wrtIn_intv_cfg_s {
    //int start;//-1 means current frame_id
    int step;//interval frame number between two writing
    int total_frame_num;//total frame number to write
}rk_aiq_uapiV2_awb_wrtIn_intv_cfg_t;
...
typedef struct rk_aiq_uapiV2_awb_wrtIn_attr_s {
    ...
    char path[100];
    int call_cnt;
    rk_aiq_uapiV2_awb_wrtIntvel_mode_e capture_mode;
    rk_aiq_uapiV2_awb_wrtIn_intv_cfg_t capture_intv_cfg;
...
typedef struct awb_measure_wp_res_fl_s {
    unsigned int WpNo;
    float gain[AWB_CHANNEL_MAX];//mean rgain  value
    int WpNoHist_2[RK_AIQ_AWB_WP_HIST_BIN_NUM]; //for test
} awb_measure_wp_res_fl_t;

typedef struct awb_measure_blk_res2_fl_s {
    unsigned int WpNo;
    float gain[AWB_CHANNEL_MAX];//mean rgain  value
} awb_measure_blk_res2_fl_t;

typedef struct awb_measure_wp_res_light_fl_s {
    awb_measure_wp_res_fl_t xYType[RK_AIQ_AWB_XY_TYPE_MAX_V200];
    float preWbgainSw[AWB_CHANNEL_MAX];
} awb_measure_wp_res_light_fl_t;

typedef struct rk_aiq_awb_stat_res_full2_s {
    //method1
    awb_measure_wp_res_light_fl_t light[CALD_AWB_LS_NUM_MAX];
    bool                          lightWpResVaLidIll[CALD_AWB_LS_NUM_MAX];//for time share
    bool                          lightWpResVaLid;//for time share
    char                          timeSignGroup[MAX_AWB_TIME_SHARE_FRAME_NUM][64];//for time share
    int WpNo[2];//for diff xy type
    int WpNo2[CALD_AWB_LS_NUM_MAX];//for diff illu
    ...
    //other
    bool                     blkWpResVaLid;
    awb_measure_blk_res2_fl_t   blkWpResult[RK_AIQ_AWB_GRID_NUM_TOTAL];

    //window in pixel domain
    //AwbMeasureWpResult_t multiwindowLightResult[RK_AIQ_AWB_MAX_WHITEREGIONS_NUM];

    //window in xy or uv domain
    bool excWpResValid;
    awb_measure_wp_res_fl_t excWpRangeResult[MAX_AWB_TIME_SHARE_FRAME_NUM*AWBSTATS_WPFLTOUT_TOTAL_ENTITY_NUM];
    awb_measure_wp_res_fl_t extraLightResult;

    //wpno histogram
    unsigned int WpNoHist[RK_AIQ_AWB_WP_HIST_BIN_NUM];
    unsigned int WpNoHistGroup[MAX_AWB_TIME_SHARE_FRAME_NUM][RK_AIQ_AWB_WP_HIST_BIN_NUM];//for time share
} rk_aiq_awb_stat_res_full2_t;
