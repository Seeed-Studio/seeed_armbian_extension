...
XCamReturn translateMultiAecStats(AiqStatsTranslator_t* pStatsTrans, const aiq_VideoBuffer_t* from,
    ...
    if (AeSwapMode == AEC_RAWSWAP_MODE_S_LITE) {
        meas_type = (left_stats->meas_type & ISP33_STAT_RAWAE0) && (left_stats->meas_type & ISP35_STAT_RAWHST0) && \
                    (right_stats->meas_type & ISP33_STAT_RAWAE0) && (right_stats->meas_type & ISP35_STAT_RAWHST0);
    } else if (AeSwapMode == AEC_RAWSWAP_MODE_M_LITE) {
        meas_type = (left_stats->meas_type & ISP33_STAT_RAWAE3) && (left_stats->meas_type & ISP33_STAT_RAWHST3) && \
                    (right_stats->meas_type & ISP33_STAT_RAWAE3) && (right_stats->meas_type & ISP33_STAT_RAWHST3);
    } else {
...
XCamReturn translateAecStatsV33(AiqStatsTranslator_t* pStatsTrans, const aiq_VideoBuffer_t* from,
    ...
    if (AeSwapMode == AEC_RAWSWAP_MODE_S_LITE) {
        meas_type = (stats->meas_type & ISP33_STAT_RAWAE0) && (stats->meas_type & ISP33_STAT_RAWHST0);
    } else if (AeSwapMode == AEC_RAWSWAP_MODE_M_LITE) {
        meas_type = (stats->meas_type & ISP33_STAT_RAWAE3) && (stats->meas_type & ISP33_STAT_RAWHST3);
    } else {
        LOGE("wrong AeSwapMode=%d\n", AeSwapMode);
        return XCAM_RETURN_ERROR_PARAM;
    }
    ...
    if(pStatsTrans->_ispParams->ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_indWk_mode || \
        ...
        statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.subWin[0].hw_ae_sumBayerR_val = \
            CLIP((s64)(((u64)stats->stat.rawae0.wnd1_sumr * awb1_gain.r / 256 - (pixel_num >> 2) * bls1_val.r) * lsc_coef[0]), 0, MAX_29BITS);
        statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.subWin[0].hw_ae_sumBayerGrGb_val = \
            CLIP((s64)(((u64)stats->stat.rawae0.wnd1_sumg * awb1_gain.gr / 256 - (pixel_num >> 1) * bls1_val.gb) * lsc_coef[1]), 0, MAX_32BITS);
        statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.subWin[0].hw_ae_sumBayerB_val = \
            CLIP((s64)(((u64)stats->stat.rawae0.wnd1_sumb * awb1_gain.b / 256 - (pixel_num >> 2) * bls1_val.b) * lsc_coef[2]), 0, MAX_29BITS);
