...
translateMultiAecStatsV39(AiqStatsTranslator_t* pStatsTrans, const aiq_VideoBuffer_t* from,
    ...
    if (AeSwapMode == AEC_RAWSWAP_MODE_S_LITE) {
        meas_type = (left_stats->meas_type & ISP39_STAT_RAWAE0) && (left_stats->meas_type & ISP39_STAT_RAWHST0) && \
                    (right_stats->meas_type & ISP39_STAT_RAWAE0) && (right_stats->meas_type & ISP39_STAT_RAWHST0);
    } else if (AeSwapMode == AEC_RAWSWAP_MODE_M_LITE) {
        meas_type = (left_stats->meas_type & ISP39_STAT_RAWAE3) && (left_stats->meas_type & ISP39_STAT_RAWHST3) && \
                    (right_stats->meas_type & ISP39_STAT_RAWAE3) && (right_stats->meas_type & ISP39_STAT_RAWHST3);
    } else {
...
XCamReturn translateAecStatsV39(AiqStatsTranslator_t* pStatsTrans, const aiq_VideoBuffer_t* from,
    ...
    if (AeSwapMode == AEC_RAWSWAP_MODE_S_LITE) {
        meas_type = (stats->meas_type & ISP39_STAT_RAWAE0) && (stats->meas_type & ISP39_STAT_RAWHST0);
    } else if (AeSwapMode == AEC_RAWSWAP_MODE_M_LITE) {
        meas_type = (stats->meas_type & ISP39_STAT_RAWAE3) && (stats->meas_type & ISP39_STAT_RAWHST3);
    } else {
        LOGE("wrong AeSwapMode=%d\n", AeSwapMode);
        return XCAM_RETURN_ERROR_PARAM;
    }
