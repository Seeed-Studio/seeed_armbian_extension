...
static bool _camGroupHandler_isBtnrLogdomain(AiqAlgoCamGroupHandler_t* pAlgoCamGroupHandler)
{
...
XCamReturn AiqAlgoCamGroupHandler_prepare(AiqAlgoCamGroupHandler_t* pAlgoCamGroupHandler, AiqCore_t* aiqCore)
{
    ...
    prepareCfg->aec.isBtnrLogdomain = _camGroupHandler_isBtnrLogdomain(pAlgoCamGroupHandler);
...
XCamReturn AiqAlgoCamGroupHandler_processing(AiqAlgoCamGroupHandler_t* pAlgoCamGroupHandler, rk_aiq_singlecam_3a_result_t** params_res_array)
{
    ...
    if (sharedCom->working_mode == RK_AIQ_WORKING_MODE_NORMAL)
        com->u.proc.isoCIS = procIn->camgroupParmasArray[0]->hdrIsoWithoutIspdgain;
    else
        com->u.proc.isoCIS = procIn->camgroupParmasArray[0]->hdrIso;
