...
void JudgeWinLocationVerticalV351s(
    struct isp2x_window* ori_win,
    WinSplitMode* mode,
    Rectangle_t left_isp_rect_,
    Rectangle_t right_isp_rect_
...
static void MergeAecBigWinStatsV351s(
...
static void MergeAecBigWinStatsVerticalV351s(
    struct isp33_rawae_stat* left,
    struct isp33_rawae_stat* right,
    struct isp33_rawae_stat* out,
    WinSplitMode             mode
...
#if defined(ISP_HW_V351s) && defined(USE_NEWSTRUCT)
...
static XCamReturn
translateMultiAecStatsV351s(AiqStatsTranslator_t* pStatsTrans, const aiq_VideoBuffer_t* from,
                          aiq_stats_base_t* to) {

    XCamReturn ret = XCAM_RETURN_NO_ERROR;

#if defined(ISP_HW_V351s) && defined(RKAIQ_HAVE_MULTIISP)

    struct isp351s_isp_meas_cfg* isp_params = &pStatsTrans->_ispParams->meas;
    ...
    JudgeWinLocationV351s(&ori_win, &AeWinSplitMode[0], pStatsTrans->left_isp_rect_, pStatsTrans->right_isp_rect_);
    ...
    JudgeWinLocationV351s(&ori_win, &AeWinSplitMode[1], pStatsTrans->left_isp_rect_, pStatsTrans->right_isp_rect_);
    ...
    JudgeWinLocationV351s(&ori_win, &HistWinSplitMode[0], pStatsTrans->left_isp_rect_, pStatsTrans->right_isp_rect_);
    ...
    JudgeWinLocationV351s(&ori_win, &HistWinSplitMode[1], pStatsTrans->left_isp_rect_, pStatsTrans->right_isp_rect_);
    ...
    if (ispUniteMode == RK_AIQ_ISP_UNITE_MODE_FOUR_GRID) {
        ...
        ori_win.h_size = isp_params->rawae0.win0_h_size;
        ori_win.v_size = isp_params->rawae0.win0_v_size;
        JudgeWinLocationVerticalV351s(&ori_win, &AeWinSplitModeV[0], pStatsTrans->left_isp_rect_, pStatsTrans->bottom_left_isp_rect_);

        ori_win.h_offs = isp_params->rawae3.win0_h_offset;
        ori_win.v_offs = isp_params->rawae3.win0_v_offset;
        ori_win.h_size = isp_params->rawae3.win0_h_size;
        ori_win.v_size = isp_params->rawae3.win0_v_size;
        JudgeWinLocationVerticalV351s(&ori_win, &AeWinSplitModeV[1], pStatsTrans->left_isp_rect_, pStatsTrans->bottom_left_isp_rect_);

        ori_win.h_offs = isp_params->rawhist0.h_offset;
        ori_win.v_offs = isp_params->rawhist0.v_offset;
        ori_win.h_size = isp_params->rawhist0.h_size;
        ori_win.v_size = isp_params->rawhist0.v_size;
        JudgeWinLocationVerticalV351s(&ori_win, &HistWinSplitModeV[0], pStatsTrans->left_isp_rect_, pStatsTrans->bottom_left_isp_rect_);

        ori_win.h_offs = isp_params->rawhist3.h_offset;
        ori_win.v_offs = isp_params->rawhist3.v_offset;
        ori_win.h_size = isp_params->rawhist3.h_size;
        ori_win.v_size = isp_params->rawhist3.v_size;
        JudgeWinLocationVerticalV351s(&ori_win, &HistWinSplitModeV[1], pStatsTrans->left_isp_rect_, pStatsTrans->bottom_left_isp_rect_);
    } else {
        for (int i = 0; i < 2; i++) {
    ...
    struct isp351s_awb_gain_cfg* awb_gain_cfg = &pStatsTrans->_ispParams->awb_gain_cfg;
    ...
    struct rkisp351s_stat_buffer* left_stats =
        (struct rkisp351s_stat_buffer*)(AiqV4l2Buffer_getExpbufUsrptr((AiqV4l2Buffer_t*)from));
    ...
    uint32_t bufCnt = bufLen / sizeof(struct rkisp351s_stat_buffer) > 4 ? 4 : bufLen / sizeof(struct rkisp351s_stat_buffer);
    uint32_t bufSize = bufLen / bufCnt;
    struct rkisp351s_stat_buffer* right_stats = (struct rkisp351s_stat_buffer*)((char*)left_stats + bufSize);
    ...
    struct rkisp351s_stat_buffer* bottom_left_stats = NULL;
    struct rkisp351s_stat_buffer* bottom_right_stats = NULL;
    if (ispUniteMode == RK_AIQ_ISP_UNITE_MODE_FOUR_GRID) {
        bottom_left_stats = (struct rkisp351s_stat_buffer*)((char*)left_stats + bufSize * 2);
        if (bottom_left_stats == NULL) {
            LOGE("fail to get stats ,ignore\n");
            return XCAM_RETURN_BYPASS;
        }

        bottom_right_stats = (struct rkisp351s_stat_buffer*)((char*)left_stats + bufSize * 3);
    ...
    if (Bnr20bitEn) {
        ...
        if (AeWinSplitMode[0] != LEFT_MODE) {
            MergeAecBigWinStatsV351s(&left_stats->stat.rawae0, &right_stats->stat.rawae0, &left_stats->stat.rawae0, AeWinSplitMode[0]);
            MergeAecBigWinStatsV351s(&left_stats->stat.rawae3, &right_stats->stat.rawae3, &left_stats->stat.rawae3, AeWinSplitMode[0]);
        }
        if (ispUniteMode == RK_AIQ_ISP_UNITE_MODE_FOUR_GRID) {
            if (AeWinSplitMode[0] != LEFT_MODE) {
                MergeAecBigWinStatsV351s(&bottom_left_stats->stat.rawae0, &bottom_right_stats->stat.rawae0, &bottom_left_stats->stat.rawae0, AeWinSplitMode[0]);
                MergeAecBigWinStatsV351s(&bottom_left_stats->stat.rawae3, &bottom_right_stats->stat.rawae3, &bottom_left_stats->stat.rawae3, AeWinSplitMode[0]);
            }

            if (AeWinSplitModeV[0] != TOP_MODE) {
                MergeAecBigWinStatsVerticalV351s(&left_stats->stat.rawae0, &bottom_left_stats->stat.rawae0, &left_stats->stat.rawae0, AeWinSplitModeV[0]);
                MergeAecBigWinStatsVerticalV351s(&left_stats->stat.rawae3, &bottom_left_stats->stat.rawae3, &left_stats->stat.rawae3, AeWinSplitModeV[0]);
            }
        }
        ...
        // aeStats_wb1OutLow_mode/aeStats_wb1OutHigh_mode
        bls1_val.r = 0;
        bls1_val.gr = 0;
        bls1_val.gb = 0;
        bls1_val.b = 0;
        ...
        if(pStatsTrans->_ispParams->ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_indWk_mode || \
            ...
            if (pStatsTrans->_ispParams->ae_cfg_v39.entityGroup.entities.entity0.hw_aeCfg_statsSrc_mode == aeStats_wb1Out_mode) {
                isp_ob_offset_rb = 0;
                isp_ob_offset_g  = 0;
            ...
            //RAWAE0 BIG-MODE
            calcAecBigWinStatsV351s(&left_stats->stat.rawae0, &statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.mainWin,
        ...
        if(pStatsTrans->_ispParams->ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_indWk_mode || \
            ...
            calcAecBigWinStatsV351s(&left_stats->stat.rawae3, &statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity3.mainWin,
                                  bls1_val, awb1_gain, lsc_coef);
    ...
    // expsoure params
    if (pStatsTrans->_ispParams->isPreIspDgain && pStatsTrans->_ispParams->ae_cfg_v39.entityGroup.entities.entity0.hw_aeCfg_statsSrc_mode == aeStats_wb1Out_mode) {
        if (pStatsTrans->_expParamsPre) {
            statsInt->aec_stats_v25.ae_exp = pStatsTrans->_expParamsPre->aecExpInfo;
        }
...
XCamReturn translateAecStatsV351s(AiqStatsTranslator_t* pStatsTrans, const aiq_VideoBuffer_t* from,
                                aiq_stats_base_t* to) {
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ...
    if (pStatsTrans->mIsMultiIsp && pStatsTrans->mIspUniteMode) {
        return translateMultiAecStatsV351s(pStatsTrans, from, to);
    }
    ...
    if(Bnr20bitEn) {
        ...
        if(pStatsTrans->_ispParams->ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_indWk_mode || \
            ...
            if (pStatsTrans->_ispParams->ae_cfg_v39.entityGroup.entities.entity0.hw_aeCfg_statsSrc_mode == aeStats_entity0_chl0DpcOut_mode || \
                pStatsTrans->_ispParams->ae_cfg_v39.entityGroup.entities.entity0.hw_aeCfg_statsSrc_mode == aeStats_entity0_chl1DpcOut_mode) {

                //RAWAE0 BIG-MODE                
                calcAecBigWinStatsV351s(&stats->stat.rawae0, &statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.mainWin,
                                      bls1_val, awb1_gain,lsc_coef);
                ...
                //For isp351s, only has one sub win
                ...
                calcAecHistBinStatsV351s(stats->stat.rawhist0.bin,
                                       statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.hist.hw_ae_histBin_val,
                                       rawhist_mode, isp_ob_offset_rb, isp_ob_offset_g,
                                       bls1_ori_val, awb1_gain, is_hdr);

            } else if (pStatsTrans->_ispParams->ae_cfg_v39.entityGroup.entities.entity0.hw_aeCfg_statsSrc_mode == aeStats_wb1Out_mode) {

                //RAWAE0 BIG-MODE
                //copy all channel
                for (int i = 0; i < ISP32_RAWAEBIG_MEAN_NUM; i++) {
                    statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.mainWin.hw_ae_meanBayerR_val[i] = \
                        CLIP((int)stats->stat.rawae0.blk_y[i / 15].blk_x[i % 15].r, 0, MAX_10BITS);
                    statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.mainWin.hw_ae_meanBayerGrGb_val[i] = \
                        CLIP((int)stats->stat.rawae0.blk_y[i / 15].blk_x[i % 15].g, 0, MAX_12BITS);
                    statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.mainWin.hw_ae_meanBayerB_val[i] = \
                        CLIP((int)stats->stat.rawae0.blk_y[i / 15].blk_x[i % 15].b, 0, MAX_10BITS);
                }
                ...
                statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.subWin[0].hw_ae_sumBayerR_val = CLIP((s64)(stats->stat.rawae0.wnd1_sumr * lsc_coef[0]),0, MAX_29BITS);
                statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.subWin[0].hw_ae_sumBayerGrGb_val = CLIP((s64)(stats->stat.rawae0.wnd1_sumg * lsc_coef[1]), 0, MAX_32BITS);
                statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.subWin[0].hw_ae_sumBayerB_val = CLIP((s64)(stats->stat.rawae0.wnd1_sumb * lsc_coef[2]), 0, MAX_29BITS);

                memcpy(statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity0.hist.hw_ae_histBin_val,
                       stats->stat.rawhist0.bin, ISP39_HIST_BIN_N_MAX * sizeof(u32));
            }
        }
        ...
        if(pStatsTrans->_ispParams->ae_cfg_v39.hw_aeCfg_entityGroup_mode == aeStats_entity03_indWk_mode || \
            ...
            case AEC_RAWSEL_MODE_CHN_1:
                ...
                calcAecBigWinStatsV351s(&stats->stat.rawae3, &statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity3.mainWin,
                                      bls1_val, awb1_gain,lsc_coef);
                ...
                //RAWHIST3 BIG-MODE
                rawhist_mode = isp_params->rawhist3.mode;
                calcAecHistBinStatsV351s(stats->stat.rawhist3.bin,
                                       statsInt->aec_stats_v25.ae_data.entityGroup.entities.entity3.hist.hw_ae_histBin_val,
                                       rawhist_mode, isp_ob_offset_rb, isp_ob_offset_g,
                                       bls1_ori_val, awb1_gain, is_hdr);
                break;
...
XCamReturn translateMultiAwbStatsV35(AiqStatsTranslator_t* pStatsTrans, const aiq_VideoBuffer_t* from,
    ...
    JudgeWinLocationV351s(&ori_win, &AwbWinSplitMode, pStatsTrans->left_isp_rect_, pStatsTrans->right_isp_rect_);
    JudgeWinLocationVerticalV351s(&ori_win, &AwbWinSplitModeV, pStatsTrans->left_isp_rect_, pStatsTrans->bottom_left_isp_rect_);
...
XCamReturn AiqStatsTranslator_init(AiqStatsTranslator_t* pStatsTrans) {
    pStatsTrans->translateAecStats          = translateAecStatsV351s;
