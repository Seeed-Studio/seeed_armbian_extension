...
#include "aiq_core_c/aiq_global_3a_info.h"
...
static void AiqCore_updateGlobalAwbInfo(AiqCore_t* pAiqCore, const aiq_stats_base_t* awbStats) {
    uint32_t camId;
    aiq_awb_stats_wrapper_t* wrap;

#ifdef RKAIQ_ENABLE_CAMGROUP
    if (pAiqCore->mCamGroupCoreManager) return;
#endif

    if (!awbStats || !awbStats->bValid) return;

    /* Check if master cam sync has been configured */
    if (aiqGlobal3AInfoMgr_getMasterCamId() == UINT32_MAX) return;

    camId = (uint32_t)AiqCore_getCamPhyId(pAiqCore);
    wrap  = (aiq_awb_stats_wrapper_t*)awbStats->_data;
    if (!wrap) return;

    aiqGlobal3AInfoMgr_pushAwbStats(camId, awbStats->frame_id, wrap);
}

void AiqCore_updateGlobalAwbResult(AiqCore_t* pAiqCore, uint32_t frame_id,
    ...
    if (!pAiqCore || !awbRes) return;

    /* Check if master cam sync has been configured */
    if (aiqGlobal3AInfoMgr_getMasterCamId() == UINT32_MAX) return;

    camId = (uint32_t)AiqCore_getCamPhyId(pAiqCore);
    aiqGlobal3AInfoMgr_pushAwbResult(camId, frame_id, awbRes);
}
...
#define NEW_PARAMS_POOL(RESULT, PTYPE)                                                     \
    ...
    case RK_AIQ_ALGO_TYPE_AF:
        ...
        NEW_PARAMS_POOL(RESULT_TYPE_PDAF_TYPE1_PARAM, rk_aiq_pdtype1_params_t);
#endif
...
XCamReturn AiqCore_init(AiqCore_t* pAiqCore, const char* sns_ent_name,
    ...
    aiqGlobal3AInfoMgr_init();
#if 0
    // FIXME: Test only! Remove before release.
    uint32_t sync_cam_ids[] = {0, 1};
    aiqGlobal3AInfoMgr_setMasterCamSyncList(0, sync_cam_ids, 2);
#endif
    aiqGlobal3AInfoMgr_setRequiredMask(AIQ_G3A_VALID_AWB_RESULT | AIQ_G3A_VALID_AWB_STATS);
    ...
    pAiqCore->mAlogsComSharedParams.aibnr_fixIndex = -1;
...
static AiqFullParams_t* analyzeInternal(AiqCore_t * pAiqCore,
    ...
    for (int i = 0; i < cnts; i++) {
        AiqAlgoHandler_t* pHdl = pHdlList[i];
        bool got_buffer        = false;
        if (grp_type == RK_AIQ_CORE_ANALYZE_AFD) {
            got_buffer = true;
        }
        while (pHdl) {
            if (pHdl->mEnable) {
...
XCamReturn AiqCore_prepare(AiqCore_t * pAiqCore,
    ...
    if ((pAiqCore->mState == RK_AIQ_CORE_STATE_STOPED) ||
        ...
        if (pAiqCore->mAlogsComSharedParams.hdr_mode == hdr_mode) {
    ...
    AiqCore_runInitialAnalyze(pAiqCore);
...
XCamReturn AiqCore_runInitialAnalyze(AiqCore_t* pAiqCore) {
    // run algos without stats to generate initial params
...
void AiqCore_setInitExp(AiqCore_t* pAiqCore, const ae_linInitExp_t* initExp) {
    if (!initExp) {
        pAiqCore->mHasInitExp = false;
        return;
    }
    pAiqCore->mInitExp    = *initExp;
    pAiqCore->mHasInitExp = true;
}
...
static XCamReturn handleAwbStats(AiqCore_t* pAiqCore, const aiq_VideoBuffer_t* buffer) {
    ...
    AiqCore_updateGlobalAwbInfo(pAiqCore, awbStat);
...
static bool _check_stats_valid(aiq_VideoBuffer_t* buffer, AIQ_STATS_TYPE_t check_type)
{
    ...
    struct rkisp351s_stat_buffer* stats =
        (struct rkisp351s_stat_buffer*)(AiqV4l2Buffer_getExpbufUsrptr((AiqV4l2Buffer_t*)buffer));
#else
...
XCamReturn AiqCore_pushStats(AiqCore_t * pAiqCore, AiqHwEvt_t * evt) {
    ...
    if (buffer && buffer->_buf_type == ISP_POLL_3A_STATS) {
        ...
        if (_check_stats_valid(buffer, AIQ_STATS_TYPE_AWB)) {
            ...
            if (delta > 3 && !pAiqCore->mIsVicapVirtualSensor) {
...
#define CLEAN_EXPS(EXP)                        \
    ...
    XCamReturn ret                      = XCAM_RETURN_NO_ERROR;
    int hdr_iso[3]                      = {0};
    int hdr_isoWithoutIspdgain[3]       = {0};
    ...
    while (id <= maxId) {
        ...
        if (pAiqCore->mAlogsComSharedParams.hdr_mode == 0) {
           ...
           hdr_isoWithoutIspdgain[0] = 50 * curExpParams->aecExpInfo.LinearExp.exp_real_params.analog_gain *
                         curExpParams->aecExpInfo.LinearExp.exp_real_params.digital_gain;
        } else {
            for (int i = 0; i < 3; i++) {
                ...
                hdr_isoWithoutIspdgain[i] = 50 * curExpParams->aecExpInfo.HdrExp[i].exp_real_params.analog_gain *
                             curExpParams->aecExpInfo.HdrExp[i].exp_real_params.digital_gain;
            }
        }
        ...
        sofInfo->isoWithoutIspdgain  = hdr_isoWithoutIspdgain[pAiqCore->mAlogsComSharedParams.hdr_mode];
...
XCamReturn AiqCore_pushEvts(AiqCore_t* pAiqCore, AiqHwEvt_t* evts) {
    ...
    if (evts->type == V4L2_EVENT_FRAME_SYNC) {
        ...
        if (delta > (pAiqCore->mDefaultDelayCnt + 1) && !pAiqCore->mIsVicapVirtualSensor) {
...
void AiqCore_setVicapVirtualSensor(AiqCore_t* pAiqCore, bool isVirtual) {
    pAiqCore->mIsVicapVirtualSensor = isVirtual;
}

bool AiqCore_isVicapVirtualSensor(AiqCore_t* pAiqCore) {
    if (!pAiqCore) return false;
    return pAiqCore->mIsVicapVirtualSensor;
}
...
XCamReturn AiqCore_setTranslaterIspUniteInfo(AiqCore_t* pAiqCore, RkAiqIspUniteInfo_t *info) {

    if (pAiqCore->mTranslator) {
        pAiqCore->mTranslator->mIspUniteInfo = *info;
        pAiqCore->mTranslator->mIspUniteMode = info->mode;
        if (pAiqCore->mTranslator->mIspUniteMode != RK_AIQ_ISP_UNITE_MODE_NORMAL) {
            pAiqCore->mTranslator->mIsMultiIsp   = true;
            if (info->mode < RK_AIQ_ISP_UNITE_MODE_3X3) {
                AiqStatsTranslator_SetLeftIspRect(pAiqCore->mTranslator, &info->isp_rect[0][0]);
                AiqStatsTranslator_SetRightIspRect(pAiqCore->mTranslator, &info->isp_rect[0][1]);
                AiqStatsTranslator_SetBottomLeftIspRect(pAiqCore->mTranslator, &info->isp_rect[1][0]);
                AiqStatsTranslator_SetBottomRightIspRect(pAiqCore->mTranslator, &info->isp_rect[1][1]);
            }
        }
    }
...
XCamReturn AiqCore_setMasterCamSyncList(uint32_t master_cam_id, const uint32_t* sync_cam_ids,
                                        uint32_t sync_cam_num) {
    return aiqGlobal3AInfoMgr_setMasterCamSyncList(master_cam_id, sync_cam_ids, sync_cam_num);
}
