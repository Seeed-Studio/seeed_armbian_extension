...
static XCamReturn _handlerAf_prepare(AiqAlgoHandler_t* pAlgoHandler) {
    ...
    // for otp
    if (pAfHdl->api_otp_af.flag != 0) {
        af_config_int->otp_af = &pAfHdl->api_otp_af;
        LOGI_AF("use otp af info from user api");
    } else {
        af_config_int->otp_af = sharedCom->snsDes.otp_af;
        LOGI_AF("use otp af info from driver");
    }
    if (pAfHdl->api_otp_pdaf.flag != 0) {
        af_config_int->otp_pdaf = &pAfHdl->api_otp_pdaf;
        LOGI_AF("use otp pdaf info from user api");
    } else {
        af_config_int->otp_pdaf = sharedCom->snsDes.otp_pdaf;
        LOGI_AF("use otp pdaf info from driver");
    }
...
#if RKAIQ_HAVE_PDAF
    aiq_params_base_t* pPdType1 = shared->fullParams->pParamsArray[RESULT_TYPE_PDAF_TYPE1_PARAM];
    if (pPdType1)
        af_proc_res_int->pdaf_type1_param = (rk_aiq_pdtype1_params_t*)pPdType1->_data;
    else
#else
        af_proc_res_int->pdaf_type1_param = NULL; 
#endif
...
static XCamReturn _handlerAf_genIspResult(AiqAlgoHandler_t* pAlgoHandler, AiqFullParams_t* params,
...
#if RKAIQ_HAVE_PDAF
    aiq_params_base_t* pPdT1Base = params->pParamsArray[RESULT_TYPE_PDAF_TYPE1_PARAM];
    if (!pPdT1Base) {
        LOGW_AF("no pdaf type1 buf !");
        return XCAM_RETURN_BYPASS;
    }
#endif
...
#if RKAIQ_HAVE_PDAF
    if (af_com->pdaf_type1_update) {
        pAfHdl->mAPdafType1ResSyncFalg = shared->frameId;
        pPdT1Base->sync_flag = pAfHdl->mAPdafType1ResSyncFalg;
        cur_params->pParamsArray[RESULT_TYPE_PDAF_TYPE1_PARAM] = params->pParamsArray[RESULT_TYPE_PDAF_TYPE1_PARAM];
        pPdT1Base->is_update = true;
        af_com->pdaf_type1_update = false;
        LOGD_AF("[%d] pdaf type1 params from algo", pAfHdl->mAPdafType1ResSyncFalg);
    } else if (pAfHdl->mAPdafType1ResSyncFalg != pPdT1Base->sync_flag) {
        pPdT1Base->sync_flag = pAfHdl->mAPdafType1ResSyncFalg;
        // copy from latest result
        if (cur_params->pParamsArray[RESULT_TYPE_PDAF_TYPE1_PARAM]) {
            pPdT1Base->is_update = true;
            *(rk_aiq_pdtype1_params_t*)(pPdT1Base->_data) =
                *(rk_aiq_pdtype1_params_t*)(cur_params->pParamsArray[RESULT_TYPE_PDAF_TYPE1_PARAM]->_data);
        } else {
            LOGE_AF("no pdaf type1 latest params !");
            pPdT1Base->is_update = false;
        }
        LOGD_AF("[%d] pdaf type1 params from latest [%d]", shared->frameId, pAfHdl->mAPdafType1ResSyncFalg);
    } else {
        pPdT1Base->is_update = false;
        // do nothing, result in buf needn't update
        //LOGD_AF("[%d] focus params needn't update", shared->frameId);
...

    {
        ...
        if (sharedCom->snsDes.lens_des.focus_support) {
            rk_aiq_uapi_af_getFocusRange(pAlgoHandler->mAlgoCtx, &range);
            rk_aiq_uapi_af_getSearchResult(pAlgoHandler->mAlgoCtx, &result);
            if (result.stat == RK_AIQ_AF_SEARCH_END) {
                sharedCom->af_converged = true;
            } else {
                sharedCom->af_converged = false;
            }

            motor_range = range.max_pos - range.min_pos;
            if (result.final_pos >= 0) {
                sharedCom->af_focuspos = (float)(result.final_pos - range.min_pos) / motor_range;
            } else {
                sharedCom->af_focuspos = -1;
            }

            LOGD_AF("af_converged %d, af_focuspos %f, pos: %d, %d, %d",
                sharedCom->af_converged, sharedCom->af_focuspos,
                result.final_pos, range.min_pos, range.max_pos);
        } else {
            sharedCom->af_converged = true;
            sharedCom->af_focuspos = 1; // 0 near, 1 far
        }
    }
    aiqMutex_lock(&pAfHdl->mPdLibOutputMutex);
    aiqCond_broadcast(&pAfHdl->mPdLibOutputCond);
...
XCamReturn AiqAlgoHandlerAf_setOtpAf(AiqAlgoHandlerAf_t* pAfHdl, struct rkmodule_af_inf *otp_af, struct rkmodule_pdaf_inf *otp_pdaf)
{
    ENTER_ANALYZER_FUNCTION();
    ...
    if (otp_af == NULL || otp_pdaf == NULL) {
        EXIT_ANALYZER_FUNCTION();
        return XCAM_RETURN_ERROR_FAILED;
    }

    aiqMutex_lock(&pHdl->mCfgMutex);
    pAfHdl->api_otp_af = *otp_af;
    pAfHdl->api_otp_pdaf = *otp_pdaf;
    aiqMutex_unlock(&pHdl->mCfgMutex);
