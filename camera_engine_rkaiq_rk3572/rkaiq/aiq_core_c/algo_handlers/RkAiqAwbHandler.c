...
static XCamReturn _handlerAwb_processing(AiqAlgoHandler_t* pAlgoHandler) {
    ...
    awb_proc_int->af_converged = sharedCom->af_converged;
    awb_proc_int->af_focuspos = sharedCom->af_focuspos;
...
#ifdef ISP_HW_V351s
        ablcProcResV32.blc_r = (short)(blc_res->calibDyn.obMean.hw_blcC_obMeanR_val + blc_res->dyn.obcPreTnr.hw_blcC_obR_offset);
        ablcProcResV32.blc_gr = (short)(blc_res->calibDyn.obMean.hw_blcC_obMeanGr_val + blc_res->dyn.obcPreTnr.hw_blcC_obGr_offset);
        ablcProcResV32.blc_gb = (short)(blc_res->calibDyn.obMean.hw_blcC_obMeanGb_val + blc_res->dyn.obcPreTnr.hw_blcC_obGb_offset);
        ablcProcResV32.blc_b = (short)(blc_res->calibDyn.obMean.hw_blcC_obMeanB_val + blc_res->dyn.obcPreTnr.hw_blcC_obB_offset);
#else
...
#ifdef ISP_HW_V351s
			uint16_t autoOB_offset = CLIP(blc_res->calibDyn.obSigma.hw_blcC_obSigmaR_val, 0, 511);
            ablcProcResV32.isp_ob_offset = autoOB_offset;
#else
    ...
    if (!AiqCore_isGroupAlgo(pAlgoHandler->mAiqCore, pAlgoHandler->mDes->type)) {
        ...
        for (uint32_t i = 0; i < RK_AIQ_CAM_GROUP_MAX_CAMS; ++i)
            awb_proc_int->global3AInfo[i] = NULL;
        ret = aiqGlobal3AInfo_getOthers(sharedCom->mCamPhyId, awb_proc_int->global3AInfo,
                                        RK_AIQ_CAM_GROUP_MAX_CAMS, &out_num);
        if (ret != XCAM_RETURN_NO_ERROR) {
            for (uint32_t i = 0; i < RK_AIQ_CAM_GROUP_MAX_CAMS; ++i)
                awb_proc_int->global3AInfo[i] = NULL;
        }

        awb_proc_int->global3AInfo_count = out_num;
        awb_proc_int->masterCamId        = aiqGlobal3AInfoMgr_getMasterCamId();

        LOGV_ANALYZER("get %d global3AInfo for awb, masterCamId=%u", out_num,
                      awb_proc_int->masterCamId);
    } else {
        for (uint32_t i = 0; i < RK_AIQ_CAM_GROUP_MAX_CAMS; ++i)
            awb_proc_int->global3AInfo[i] = NULL;

        awb_proc_int->global3AInfo_count = 0;
    }
    ...
    if (awb_proc_res_int->awb_cfg_update || awb_proc_res_int->awb_gain_update) {
		...
		if (pAwbHdl->mProcResShared) {
			...
			pAwbShared->awbConverged = awb_proc_res_int->awbConverged;
                ...
                // Update global 3A AWB result
                if (!AiqCore_isGroupAlgo(pAlgoHandler->mAiqCore, pAlgoHandler->mDes->type))
                    AiqCore_updateGlobalAwbResult(pAlgoHandler->mAiqCore, shared->frameId,
                                                  awb_proc_res_int);
    } else {
