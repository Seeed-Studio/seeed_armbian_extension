...
static bool _handlerAe_isBtnrLogdomain(AiqAlgoHandler_t* pAlgoHandler)
{
    RkAiqAlgosComShared_t* sharedCom = &pAlgoHandler->mAiqCore->mAlogsComSharedParams;

    btnr_api_attrib_t* btnr_attrib =
        (btnr_api_attrib_t*)(CALIBDBV2_GET_MODULE_PTR((void*)sharedCom->calibv2, bayertnr));

    if (btnr_attrib) {
        if (btnr_attrib->en && !btnr_attrib->bypass) {
#if RKAIQ_HAVE_BAYERTNR_V30
            if (btnr_attrib->opMode == RK_AIQ_OP_MODE_AUTO)
                return (btnr_attrib->stAuto.dyn[0].sigmaEnv.hw_btnrC_sigmaAttrib.hw_btnrCfg_pixDomain_mode == btnr_pixLog2Domain_mode);

            else
                return (btnr_attrib->stMan.dyn.sigmaEnv.hw_btnrC_sigmaAttrib.hw_btnrCfg_pixDomain_mode == btnr_pixLog2Domain_mode);
#else
            if (btnr_attrib->opMode == RK_AIQ_OP_MODE_AUTO)
                return (btnr_attrib->stAuto.spNrDyn[0].sigmaEnv.hw_btnrC_sigmaAttrib.hw_btnrCfg_pixDomain_mode == btnr_pixLog2Domain_mode);

            else
                return (btnr_attrib->stMan.spNrDyn.sigmaEnv.hw_btnrC_sigmaAttrib.hw_btnrCfg_pixDomain_mode == btnr_pixLog2Domain_mode);
#endif
        }
    }
    return false;
...
static XCamReturn _handlerAe_prepare(AiqAlgoHandler_t* pAlgoHandler) {
    ...
    ae_config_int->lcg2lofic_ratio = sharedCom->snsDes.lcg2lofic_ratio;
    ae_config_int->lofic2vs_ratio = sharedCom->snsDes.lofic2vs_ratio;
    ae_config_int->exp_offset = sharedCom->snsDes.exp_offset;

    // only for isp351
    ae_config_int->isBtnrLogdomain = _handlerAe_isBtnrLogdomain(pAlgoHandler);
...
static XCamReturn _handlerAe_processing(AiqAlgoHandler_t* pAlgoHandler) {
    ...
    if (pAlgoHandler->mAiqCore->mAlogsComSharedParams.init) {
        ...
        if (pAlgoHandler->mAiqCore->mAlogsComSharedParams.hdr_mode == 0) {
            ...
            if (pAlgoHandler->mAiqCore->mIsVicapVirtualSensor) {
                if (pAlgoHandler->mAiqCore->mHasInitExp) {
                    ae_linInitExp_t* pInitExp = &pAlgoHandler->mAiqCore->mInitExp;
                    hdr_iso[0] =
                        50 * pInitExp->sw_aeT_initGain_val * pInitExp->sw_aeT_initIspDGain_val;
                }
            }
        } else {
            for(int i = 0; i < 3; i++) {
    ...
    translator->_faceAeCfg.en = ae_proc_res_int->ae_proc_res_rk->IsFaceAe;
    translator->_faceAeCfg.statsSrc = ae_proc_res_int->ae_proc_res_rk->FaceStatsSrc;
