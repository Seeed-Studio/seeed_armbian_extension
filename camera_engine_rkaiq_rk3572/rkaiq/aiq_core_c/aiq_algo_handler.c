...
XCamReturn AiqAlgoHandler_configInparamsCom(AiqAlgoHandler_t* pAlgoHandler, RkAiqAlgoCom* com,
    ...
    if (type == RKAIQ_CONFIG_COM_PREPARE) {
        ...
        if (sharedCom->working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
            com->u.proc.isoCIS = shared->isoWithoutIspdgain;
        }
        else {
            com->u.proc.isoCIS = shared->iso;
        }
        ...
        AiqFullParams_t* params, AiqFullParams_t* cur_params, int restype, int size)
{
    ...
    if (proc_res->cfg_update) {
        ...
        if (pCurBase) {
            ...
            LOGW("%s: no latest params !", ResTypeStr);
            //fallback in case algo didn't output initial params
            cur_params->pParamsArray[restype] = pBase;
...
XCamReturn AiqAlgoHandler_queryInterpIso_common(AiqAlgoHandler_t* pAlgoHandler,
        uint32_t* interpIso, uint32_t* isoH, uint32_t* isoL, bool isCISGain)
{
    ENTER_ANALYZER_FUNCTION();
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    AiqAlgoHandler_t* pHdl = (AiqAlgoHandler_t*)pAlgoHandler;
    ...
    if (isCISGain)
        *interpIso = shared->isoWithoutIspdgain;
    else
    ...
    if (isFixedAlgo && (sharedCom->aibnr_fixIndex != -1)) {
        ...
        *isoL = *isoH;
        ...
        float ratio = 0.0f;
        int iLow = 0, iHigh = 0;
        pre_interp(*interpIso, sharedCom->calibv2->sensor_info->iso_list, 13, &iLow, &iHigh, &ratio);
        if (iLow != -1)
