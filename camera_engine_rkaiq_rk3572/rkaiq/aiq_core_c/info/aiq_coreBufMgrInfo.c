...
void core_buf_mgr_dump_mem_pool_info(AiqCore_t* self, st_string* result) {
    ...
    __snprintf_strings("ynr_proc_res", self->mProcResYnrSharedPool, self->mNoFreeBufCnt.ynrProcRes,
                       buffer, result);
    {
        uint32_t cam_id = AiqCore_getCamPhyId(self);
        AiqPool_t* pool = NULL;
        aiqGlobal3AInfoMgr_getPoolAndMap(cam_id, &pool, NULL);
        uint32_t no_free_cnt = aiqGlobal3AInfoMgr_getNoFreeBufCnt(cam_id);
        if (pool) __snprintf_strings("g3A_info", pool, no_free_cnt, buffer, result);
    }

    aiq_string_printf(result, "\n");
}

void core_buf_mgr_dump_map_info(AiqCore_t* self, st_string* result) {
    char buffer[MAX_LINE_LENGTH] = {0};
    uint32_t cam_id              = AiqCore_getCamPhyId(self);
    AiqMap_t* map                = NULL;
    aiqGlobal3AInfoMgr_getPoolAndMap(cam_id, NULL, &map);
    if (!map) return;

    aiq_info_dump_title(result, "g3A map info");

    snprintf(buffer, MAX_LINE_LENGTH, "%-14s%-10s%-11s%-10s", "map_name", "item_num", "item_size",
             "free_num");
    aiq_string_printf(result, buffer);
    aiq_string_printf(result, "\n");

    snprintf(buffer, MAX_LINE_LENGTH, "%-14s%-10d%-11d%-10d", map->_name, aiqMap_itemNums(map),
             aiqMap_itemSize(map), aiqMap_itemNums(map) - aiqMap_size(map));
    aiq_string_printf(result, buffer);
    aiq_string_printf(result, "\n");
}
...
int core_buf_mgr_dump(void* self, st_string* result, int argc, void* argv[]) {
    ...
    core_buf_mgr_dump_map_info((AiqCore_t*)self, result);
