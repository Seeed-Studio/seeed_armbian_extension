...
void convertAiqAwbGainToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    ...
    calibdb_cis_wb_t* awb_cis_info = (calibdb_cis_wb_t*)pCvt->awb_cis_info;
    ...
    if (RK_AIQ_HDR_IS_SENSOR_BUILTIN(pCvt->_working_mode)) {
        if (awb_cis_info && awb_cis_info->setWbGnInSensor_en == false) {
            pCvt->awb_gain_final.applyPosition = IN_AWBGAIN1;
        }
    }
    // 2) //otp
    ...
    struct isp351s_awb_gain_cfg* cfg = &pCvt->isp_params.isp_cfg->others.awb_gain_cfg;
#else
    ...
    if (pCvt->awb_gain_final.applyPosition == IN_AWBGAIN0) {
...
#if defined(ISP_HW_V351s)
        cfg->gain0_r     = R > max_wb_gain ? max_wb_gain : R;
        cfg->gain0_b    = B > max_wb_gain ? max_wb_gain : B;
        cfg->gain0_gr = Gr > max_wb_gain ? max_wb_gain : Gr;
        cfg->gain0_gb = Gb > max_wb_gain ? max_wb_gain : Gb;
        cfg->gain1_r     = R > max_wb_gain ? max_wb_gain : R;
        cfg->gain1_b    = B > max_wb_gain ? max_wb_gain : B;
        cfg->gain1_gr = Gr > max_wb_gain ? max_wb_gain : Gr;
        cfg->gain1_gb = Gb > max_wb_gain ? max_wb_gain : Gb;
        cfg->gain2_r     = R > max_wb_gain ? max_wb_gain : R;
        cfg->gain2_b    = B > max_wb_gain ? max_wb_gain : B;
        cfg->gain2_gr = Gr > max_wb_gain ? max_wb_gain : Gr;
        cfg->gain2_gb = Gb > max_wb_gain ? max_wb_gain : Gb;
#else
        ...
        if(! pCvt->mCommonCvtInfo._airms_en) {
            ...
            //_airms_en=1+ linear
#if defined(ISP_HW_V351s)
            cfg->gain0_r     = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain0_b    = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain0_gr = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain0_gb = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain1_r     = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain1_b    = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain1_gr = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain1_gb = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain2_r     = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain2_b    = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain2_gr = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
            cfg->gain2_gb = (1 << ISP2X_WBGAIN_FIXSCALE_BIT);
#else
    ...
    if ((RK_AIQ_HDR_IS_SENSOR_BUILTIN(pCvt->_working_mode) &&
         (!awb_cis_info || awb_cis_info->setWbGnInSensor_en)) ||
...
static void ConfigBlc2(const blc_res_cvt_t* ablc, const awbStats_cfg_t* awb_meas, float isp_dgain,
    ...
    if (awb_meas->blc.hw_awbCfg_blc_en
        &&awb_meas->hw_awbCfg_statsSrc_mode == awbStats_drcOut_mode ) {
...
static XCamReturn ConfigOverexposureValue(const blc_res_cvt_t* ablc, float hdrmge_gain0_1,
    ...
    if (ablc && ablc->en && ablc->obcPostTnr.sw_blcT_obcPostTnr_en)
        ob = ablc->obcPostTnr.sw_blcT_autoOB_offset;
    if (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_drcOut_mode) {
        ...
        if (working_mode != RK_AIQ_WORKING_MODE_NORMAL) {
            awb_cfg_v35->in_overexposure_threshold = overexposure_value * hdrmge_gain0_1 * dgain - max_blc / 16;
            LOGD_AWB("hdrmge_gain0_1 %f, dgain %f, max_blc %d, overexposure_th %d", hdrmge_gain0_1, dgain, max_blc, awb_cfg_v35->in_overexposure_threshold);
        } else {
...
void convertAiqAwbToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    LOG1_AWB("%s enter", __FUNCTION__);
    //TODO config 16*8
    ...
    if (ae_exp != NULL) {
        if (pCvt->_working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
            ...
            float sExpo = ae_exp->HdrExp[0].exp_real_params.cis_total_gain *
                          ae_exp->HdrExp[0].exp_real_params.integration_time;

            float lExpo = ae_exp->HdrExp[1].exp_real_params.cis_total_gain *
...
void convertAiqYnrToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
void convertAiqCnrToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
void convertAiqDmToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
void convertAiqTexEstToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    memcpy(&pCvt->mCommonCvtInfo.texEst_param, pBase->_data, sizeof(texEst_param_t));
}

void convertAiqSharpToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
void convertAiqHisteqToIsp33Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
void convertAiqEnhToIsp33Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
void convertAiqHsvToIsp35Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    if (pBase->en) {
...
static const struct params_cvt_info_isp35 params_cvts_isp35[] = {
    ...
    CVT_INFO(RESULT_TYPE_AWB_PARAM, convertAiqAwbToIsp35Params),  // must place convertAiqAwbGainToIsp35Params before convertAiqAwbToIsp35Params
