...
#include "hwi_c/isp351s/aiq_isp351sParamsCvt.h"
#endif
...
void AiqIspParamsCvt_getCommonCvtInfo(AiqIspParamsCvt_t* pCvt, AiqList_t* results, bool use_aiisp, bool _airms_en, bool _aiynr_en) {
    ...
    if (aeResult) {
        ...
        if (pCvt->_working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
            ...
            int isoWithoutIspdgain = 50 * exp->aecExpInfo.LinearExp.exp_real_params.analog_gain *
                              exp->aecExpInfo.LinearExp.exp_real_params.digital_gain;

            pCvt->mCommonCvtInfo.frameIso[0]   = iso;
            pCvt->mCommonCvtInfo.frameIsoWithoutIspdgain[0] = isoWithoutIspdgain;
            ...
            for (int i = 0; i < 3; i++) {
                ...
                int isoWithoutIspdgain = 50 *
                          ae_exp->HdrExp[i].exp_real_params.analog_gain *
                          ae_exp->HdrExp[i].exp_real_params.digital_gain;

                pCvt->mCommonCvtInfo.frameIso[i] = iso;
                pCvt->mCommonCvtInfo.frameIsoWithoutIspdgain[i] = isoWithoutIspdgain;
...
#ifdef ISP_HW_V351s
        pCvt->mCommonCvtInfo.blc_res.obcPreTnr.hw_blcC_obR_val =
            (short)(blc_v32->calibDyn.obMean.hw_blcC_obMeanR_val + blc_v32->dyn.obcPreTnr.hw_blcC_obR_offset);
        pCvt->mCommonCvtInfo.blc_res.obcPreTnr.hw_blcC_obGr_val =
            (short)(blc_v32->calibDyn.obMean.hw_blcC_obMeanGr_val + blc_v32->dyn.obcPreTnr.hw_blcC_obGr_offset);
        pCvt->mCommonCvtInfo.blc_res.obcPreTnr.hw_blcC_obGb_val =
            (short)(blc_v32->calibDyn.obMean.hw_blcC_obMeanGb_val + blc_v32->dyn.obcPreTnr.hw_blcC_obGb_offset);
        pCvt->mCommonCvtInfo.blc_res.obcPreTnr.hw_blcC_obB_val =
            (short)(blc_v32->calibDyn.obMean.hw_blcC_obMeanB_val + blc_v32->dyn.obcPreTnr.hw_blcC_obB_offset);
        pCvt->mCommonCvtInfo.blc_res.obcPostTnr.sw_blcT_obcPostTnr_en =
            blc_v32->dyn.obcPostTnr.sw_blcT_obcPostTnr_en;
        pCvt->mCommonCvtInfo.blc_res.obcPostTnr.sw_blcT_autoOB_offset =
            CLIP(blc_v32->calibDyn.obSigma.hw_blcC_obSigmaR_val, 0, 511);
#else
...
#elif defined(ISP_HW_V351s)
    pCvt->isp_params.isp_cfg = (struct isp351s_isp_params_cfg*)isp_cfg;
#endif
...
#elif defined(ISP_HW_V351s)
        pCvt->mIspParamsCvtOps.update(isp_cfg, pCvt->mCvtedIsp351sPrams);
#endif
    }
#endif
...
void AiqIspParamsCvt_setCalib(AiqIspParamsCvt_t* pCvt, const CamCalibDbV2Context_t* calibv2) {
    ...
    calibdb_cis_wb_t* cis_wb = (calibdb_cis_wb_t*)(CALIBDBV2_GET_MODULE_PTR((void*)calibv2, CisWb));
    if (cis_wb) {
        pCvt->awb_cis_info = cis_wb;
    } else {
        pCvt->awb_cis_info = NULL;
    }
...
#endif
#elif defined(ISP_HW_V351s)
    pCvt->mIspParamsCvtOps.Convert3aResultsToIspCfg = Convert3aResultsToIsp351sCfg;
#if RKAIQ_HAVE_DUMPSYS
    pCvt->mCvtedIsp351sPrams =
        (struct isp351s_isp_params_cfg*)aiq_mallocz(sizeof(*pCvt->mCvtedIsp351sPrams));
    pCvt->mIspParamsCvtOps.update = AiqIspParamsCvt_updIsp351sParams;
#endif
#else
