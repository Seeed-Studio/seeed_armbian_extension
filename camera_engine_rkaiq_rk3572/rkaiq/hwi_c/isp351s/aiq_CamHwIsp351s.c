...
static void CamHwIsp351s_updateEffParams(AiqCamHwBase_t* pCamHw, void* params, void* ori_params) {
#if defined(ISP_HW_V351s)
    struct isp351s_isp_params_cfg* isp_params = (struct isp351s_isp_params_cfg*)params;
    ...
    pDstEff->lsc_cfg      = pCamHw->_mIspParamsCvt->mLatestLscCfg;
    pDstEff->lsc_en       = (pCamHw->_mIspParamsCvt->mCvtedIsp351sPrams->module_ens & ISP2X_MODULE_LSC);

    struct isp351s_bay3d_cfg* pBay3dCfg = &pCamHw->_mIspParamsCvt->mCvtedIsp351sPrams->others.bay3d_cfg;
    pDstEff->isPreIspDgain =
        (pCamHw->_mIspParamsCvt->mCommonCvtInfo.btnr_en) &&
        (pCamHw->_mIspParamsCvt->mCommonCvtInfo.sw_btnrT_outFrmBase_mode == btnr_preBaseOut_mode) &&
        !(pCamHw->_mIspParamsCvt->mCommonCvtInfo.btnrCfg_pixDomain_mode == btnr_pixLog2Domain_mode);

    bool use_isp_wbgain = !pCamHw->_airms_en || pCamHw->airms_wbgain_mode == AIRMS_WBGAIN_APPLY_ISP;
    bool no_sensor_compr = !pCamHw->mSnsDes.compr_bit;

    if (use_isp_wbgain && no_sensor_compr) {
        ...
        if (pDstEff1) {
            ...
            pDstEff1->isPreIspDgain = pDstEff->isPreIspDgain;
        } else {
            pDstEff->bls_cfg      = pCamHw->_mIspParamsCvt->mLatestBlsCfg;
            pDstEff->awb_gain_cfg = pCamHw->_mIspParamsCvt->mLatestWbGainCfg;
        }
...
#if defined(RKAIQ_HAVE_MULTIISP)
    struct isp351s_isp_params_cfg* dst_isp_params       = NULL;
    struct isp351s_isp_params_cfg* lates_isp_params_ptr = NULL;
    struct isp351s_isp_params_cfg* new_isp_params       = NULL;
...
#define UPDATE_EFF_PARAMS()                                                                       \
    ...
    if (g_mIsMultiIspMode) {
        ...
        new_isp_params = (struct isp351s_isp_params_cfg*)ori_params;
        UPDATE_EFF_PARAMS();
...
bool CamHwIsp351s_processTb(AiqCamHwBase_t* pCamHw, void* params) {
#if defined(ISP_HW_V351s)
    struct isp351s_isp_params_cfg* isp_params = (struct isp351s_isp_params_cfg*)params;
    ...
    if (pCamHw->mTbInfo.is_fastboot) {
        if (isp_params->frame_id == 0 && pCamHw->_not_skip_first) {
            ...
            if (!pCamHw->_skipped_params) {
                pCamHw->_skipped_params = aiq_mallocz(sizeof(struct isp351s_isp_params_cfg));
                *((struct isp351s_isp_params_cfg*)pCamHw->_skipped_params) = *isp_params;
            ...
            struct isp351s_isp_params_cfg* skip_param =
                (struct isp351s_isp_params_cfg*)pCamHw->_skipped_params;
            struct isp351s_isp_params_cfg* new_param  = (struct isp351s_isp_params_cfg*)params;
...
XCamReturn CamHwIsp351s_saveInfoForStartAgainTb(AiqCamHwBase_t* pCamHw) {
#if defined(ISP_HW_V351s)
...
XCamReturn CamHwIsp351s_getLastEffectParamTb(AiqCamHwBase_t* pCamHw) {
#if defined(ISP_HW_V351s)
    char fastboot_info_path[32];
    snprintf(fastboot_info_path, 32, "%s%d", AIQCAM_FASTBOOT_FILE_PREFIX, pCamHw->mCamPhyId);
...
XCamReturn AiqCamHwIsp351s_init(AiqCamHwBase_t* pCamHw, const char* sns_ent_name) {
    XCamReturn ret                    = AiqCamHwBase_init(pCamHw, sns_ent_name);
    pCamHw->mVicapIspPhyLinkSupported = true;

    pCamHw->updateEffParams      = CamHwIsp351s_updateEffParams;
    pCamHw->processTb            = CamHwIsp351s_processTb;
    pCamHw->getLastEffectParamTb = CamHwIsp351s_getLastEffectParamTb;
    pCamHw->saveInfotoFileTb     = CamHwIsp351s_saveInfoForStartAgainTb;
