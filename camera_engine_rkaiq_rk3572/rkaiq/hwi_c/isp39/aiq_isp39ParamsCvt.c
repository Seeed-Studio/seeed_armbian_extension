...
static XCamReturn ConfigOverexposureValue(const blc_res_cvt_t* ablc, float hdrmge_gain0_1,
    ...
    if (awb_meas->hw_awbCfg_statsSrc_mode == awbStats_drcOut_mode) {
        ...
        if (working_mode != RK_AIQ_WORKING_MODE_NORMAL) {
            awb_cfg_v32->in_overexposure_threshold = overexposure_value * hdrmge_gain0_1 * dgain - max_blc / 16;
            LOGD_AWB("hdrmge_gain0_1 %f, dgain %f, max_blc %d, overexposure_value %d", hdrmge_gain0_1, dgain, max_blc, overexposure_value);
        } else {
...
static void ConfigPreWbgain3(AiqIspParamsCvt_t* pCvt,struct isp39_rawawb_meas_cfg* awb_cfg_v32,
...
static void convertAiqAwbToIsp39Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    ...
    ConfigPreWbgain3(pCvt,awb_cfg_v32, awb_meas_priv, &pCvt->awb_gain_final);
...
void convertAiqAfToIsp39Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    ...
    pCvt->mLatestMeasCfg.rawaf = *rawaf;
}
#endif
...
void convertAiqExpIspDgainToIsp39Params(AiqIspParamsCvt_t* pCvt,
    ...
    if (pCvt->_working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
        ...
        if (pCvt->mCommonCvtInfo._airms_en) {
            ...
            uint16_t g           = (uint16_t)MIN(fixedGain1x * isp_dgain + 0.5f, max_wb_gain);
            ...
            dest_cfg->gain2_b  = g;
#else
            dest_cfg->gain0_red     = g;
            dest_cfg->gain0_green_r = g;
            dest_cfg->gain0_green_b = g;
            dest_cfg->gain0_blue    = g;
            dest_cfg->gain1_red     = g;
            dest_cfg->gain1_green_r = g;
            dest_cfg->gain1_green_b = g;
            dest_cfg->gain1_blue    = g;
            dest_cfg->gain2_red     = g;
            dest_cfg->gain2_green_r = g;
            dest_cfg->gain2_green_b = g;
            dest_cfg->gain2_blue    = g;
#endif
...
void convertAiqLscToIsp39Params(AiqIspParamsCvt_t* pCvt, aiq_params_base_t* pBase) {
    ...
    pCvt->mLatestLscCfg = pCvt->isp_params.isp_cfg->others.lsc_cfg;
}
#endif
