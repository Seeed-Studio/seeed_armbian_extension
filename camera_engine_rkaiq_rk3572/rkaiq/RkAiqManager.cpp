...
RkAiqManager::prepare(uint32_t width, uint32_t height, rk_aiq_working_mode_t mode)
{
    ...
    mCamHw->getIspUniteInfo(isp_unite_info);
    mRkAiqAnalyzer->setTranslaterIspUniteInfo(&isp_unite_info);
...
RkAiqManager::applyAnalyzerResult(SmartPtr<RkAiqFullParamsProxy>& results, bool ignoreIsUpdate)
{
    ...
    if (aiqParams->mPdType1Params.ptr()) {
        aiqParams->mPdType1Params->setType(RESULT_TYPE_PDAF_TYPE1_PARAM);
        results_list.push_back(aiqParams->mPdType1Params);
    }
#endif
...
void
RkAiqManager::rkAiqSetVcpSclInfo(bool en, int ratio)
{
    ENTER_XCORE_FUNCTION();

    mCamHw->setCifSclStartFlag(ratio, en);
