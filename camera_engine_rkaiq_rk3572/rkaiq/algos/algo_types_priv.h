...
#endif
#ifdef ISP_HW_V351s
#include "include/common/rk-isp351s-config.h"
#endif
...
typedef struct AiqGlobal3AInfo_s AiqGlobal3AInfo_t;
...
typedef struct _RkAiqAlgoProcAwb {
    ...
    // global 3A info
    AiqGlobal3AInfo_t* global3AInfo[RK_AIQ_CAM_GROUP_MAX_CAMS];
    uint32_t global3AInfo_count;
...
typedef struct _RkAiqAlgoProcResAwb {
    ...
    float LVValue;
    float fRGBLv[3];
    rk_aiq_wb_gain_t awb_gain_befmulcamfuse;//before mul cam fuse
...
typedef struct _RkAiqAlgoProcResAf {
    ...
    rk_aiq_pdtype1_params_t* pdaf_type1_param;
    uint32_t id;
} RkAiqAlgoProcResAf;
