...
#endif
#if ISP_HW_V351s // dm33
    out->dyn.gInterp.hw_dmT_gInterpGlbSharpStrg_en = paut->dyn[inear].gInterp.hw_dmT_gInterpGlbSharpStrg_en;
    out->dyn.cFlt.hw_dmT_cnrMedFltIndex_min = interpolation_f32(
                paut->dyn[ilow].cFlt.hw_dmT_cnrMedFltIndex_min, paut->dyn[ihigh].cFlt.hw_dmT_cnrMedFltIndex_min, ratio);
    out->dyn.cFlt.hw_dmT_cnrMedFltIndex_max = interpolation_f32(
                paut->dyn[ilow].cFlt.hw_dmT_cnrMedFltIndex_max, paut->dyn[ihigh].cFlt.hw_dmT_cnrMedFltIndex_max, ratio);
    out->dyn.cFlt.hw_dmT_cnrMedFltRange_scale = interpolation_f32(
                paut->dyn[ilow].cFlt.hw_dmT_cnrMedFltRange_scale, paut->dyn[ihigh].cFlt.hw_dmT_cnrMedFltRange_scale, ratio);
    out->dyn.cFlt.hw_dmT_cnrMedFltWgt_slope = interpolation_f32(
                paut->dyn[ilow].cFlt.hw_dmT_cnrMedFltWgt_slope, paut->dyn[ihigh].cFlt.hw_dmT_cnrMedFltWgt_slope, ratio);
    for (int i = 0; i < RK_DM23_LUMA_POINT_NUM; i++) {
        out->dyn.cFlt.hw_dmT_cnrMedFltWgt_minThred[i] = interpolation_u16(
                    paut->dyn[ilow].cFlt.hw_dmT_cnrMedFltWgt_minThred[i], paut->dyn[ihigh].cFlt.hw_dmT_cnrMedFltWgt_minThred[i], ratio);
    }
    out->dyn.cFlt.hw_dmT_cnrEdgeNorm_bit = interpolation_f32(
                paut->dyn[ilow].cFlt.hw_dmT_cnrEdgeNorm_bit, paut->dyn[ihigh].cFlt.hw_dmT_cnrEdgeNorm_bit, ratio);
#endif
    return XCAM_RETURN_NO_ERROR;
}
#endif
