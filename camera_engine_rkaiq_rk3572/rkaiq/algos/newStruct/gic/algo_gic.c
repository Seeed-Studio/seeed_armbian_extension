    ...
    int iso)
{
	...
	// gic
	for(int i = 0; i < RKGIC_V40_LUMA_POINT_NUM; i++)
	{
		out->dyn.gic.hw_gic_luma2SofThd_thred[i] = ROUND_F(interpolation_u16(
            paut->dyn[ilow].gic.hw_gic_luma2SofThd_thred[i], paut->dyn[ihigh].gic.hw_gic_luma2SofThd_thred[i], uratio));
	}

    for (int k = 0; k < 4; k++)
    {
        out->dyn.gic.hw_gic_loFltGr_coeff[k] 	= paut->dyn[inear].gic.hw_gic_loFltGr_coeff[k];
    }
    for (int k = 0; k < 2; k++)
    {
        out->dyn.gic.hw_gic_loFltGb_coeff[k] 	= paut->dyn[inear].gic.hw_gic_loFltGb_coeff[k];
    }

	//////////////////////////////////////////////////////////////////////////
	// fusion wgt
	out->dyn.gic.hw_gic_fusionWgt_slope	= interpolation_f32(
            paut->dyn[ilow].gic.hw_gic_fusionWgt_slope, paut->dyn[ihigh].gic.hw_gic_fusionWgt_slope, ratio);

	for(int i = 0; i < RKGIC_V40_LUMA_POINT_NUM; i++)
	{
		out->dyn.gic.hw_gic_fusionWgt_minThred[i]	= interpolation_u16(
            paut->dyn[ilow].gic.hw_gic_fusionWgt_minThred[i], paut->dyn[ihigh].gic.hw_gic_fusionWgt_minThred[i], uratio);
	}
	//////////////////////////////////////////////////////////////////////////
	// RB filter
	out->dyn.RBFilter.rsgma.hw_gic_rgeSgm_scale	= interpolation_f32(
            paut->dyn[ilow].RBFilter.rsgma.hw_gic_rgeSgm_scale, paut->dyn[ihigh].RBFilter.rsgma.hw_gic_rgeSgm_scale, ratio);
	out->dyn.RBFilter.gausFlt.hw_gic_gausFiltOut_alpha = interpolation_f32(
            paut->dyn[ilow].RBFilter.gausFlt.hw_gic_gausFiltOut_alpha, paut->dyn[ihigh].RBFilter.gausFlt.hw_gic_gausFiltOut_alpha, ratio);
	out->dyn.RBFilter.rat2MaxWgtThred.hw_gic_rat2MinWgt_minThred = interpolation_f32(
            paut->dyn[ilow].RBFilter.rat2MaxWgtThred.hw_gic_rat2MinWgt_minThred, paut->dyn[ihigh].RBFilter.rat2MaxWgtThred.hw_gic_rat2MinWgt_minThred, ratio);
	out->dyn.RBFilter.rat2MaxWgtThred.hw_gic_rat2MaxWgt_maxThred = interpolation_f32(
            paut->dyn[ilow].RBFilter.rat2MaxWgtThred.hw_gic_rat2MaxWgt_maxThred, paut->dyn[ihigh].RBFilter.rat2MaxWgtThred.hw_gic_rat2MaxWgt_maxThred, ratio);
	out->dyn.RBFilter.hw_gic_bfFltWgt_min = interpolation_f32(
            paut->dyn[ilow].RBFilter.hw_gic_bfFltWgt_min, paut->dyn[ihigh].RBFilter.hw_gic_bfFltWgt_min, ratio);
	out->dyn.RBFilter.hw_gic_RBFiltOut_alpha = interpolation_f32(
            paut->dyn[ilow].RBFilter.hw_gic_RBFiltOut_alpha, paut->dyn[ihigh].RBFilter.hw_gic_RBFiltOut_alpha, ratio);

	out->dyn.RBFilter.noiseBalStrg.hw_gic_RNoiseBal_strg = interpolation_f32(
            paut->dyn[ilow].RBFilter.noiseBalStrg.hw_gic_RNoiseBal_strg, paut->dyn[ihigh].RBFilter.noiseBalStrg.hw_gic_RNoiseBal_strg, ratio);
	out->dyn.RBFilter.noiseBalStrg.hw_gic_BNoiseBal_strg = interpolation_f32(
            paut->dyn[ilow].RBFilter.noiseBalStrg.hw_gic_BNoiseBal_strg, paut->dyn[ihigh].RBFilter.noiseBalStrg.hw_gic_BNoiseBal_strg, ratio);

	for(int i = 0; i < RKGIC_V40_LUMA_POINT_NUM; i++)
	{
		out->dyn.RBFilter.rsgma.hw_gic_RFlt_sigma[i] = ROUND_F(interpolation_u16(
                paut->dyn[ilow].RBFilter.rsgma.hw_gic_RFlt_sigma[i], paut->dyn[ihigh].RBFilter.rsgma.hw_gic_RFlt_sigma[i], uratio));
		out->dyn.RBFilter.rsgma.hw_gic_BFlt_sigma[i] = ROUND_F(interpolation_u16(
                paut->dyn[ilow].RBFilter.rsgma.hw_gic_BFlt_sigma[i], paut->dyn[ihigh].RBFilter.rsgma.hw_gic_BFlt_sigma[i], uratio));
	}

    for (int k = 0; k < 3; k++)
    {
        out->dyn.RBFilter.hw_gic_bfFlt_coeff[k]	= paut->dyn[inear].RBFilter.hw_gic_bfFlt_coeff[k];
        out->dyn.RBFilter.gausFlt.hw_gic_gausFlt_coeff[k] = paut->dyn[inear].RBFilter.gausFlt.hw_gic_gausFlt_coeff[k];
    }
#endif
    return XCAM_RETURN_NO_ERROR;
}
