...
#if defined(RKAIQ_HAVE_BAYERTNR_V41) || defined(RKAIQ_HAVE_BAYERTNR_V42) || defined(RKAIQ_HAVE_BAYERTNR_V51)
XCamReturn BtnrSelectParam
(
    BtnrContext_t *pBtnrCtx,
    btnr_param_t* out,
...
#if defined(RKAIQ_HAVE_BAYERTNR_V42) || defined(RKAIQ_HAVE_BAYERTNR_V51)
    ...
    int isoCIS)
{
    ...
    btnr_calib_dyn_t* pCalib = pBtnrCtx->btnr_attrib->stAuto.calibDyn;

    if(pCalib == NULL || out == NULL) {
        LOGE_ANR("%s(%d): null pointer\n", __FUNCTION__, __LINE__);
        return XCAM_RETURN_ERROR_PARAM;
    }

    pre_interp(isoCIS, pBtnrCtx->iso_list, 13, &ilow, &ihigh, &ratio);
    uratio = ratio * (1 << RATIO_FIXBIT);
    ...
    for (i = 0; i < 20; i++) {
        out->calibDyn.hw_btnrC_mdSigma_curve.idx[i] = pCalib[inear].hw_btnrC_mdSigma_curve.idx[i];
        out->calibDyn.hw_btnrC_mdSigma_curve.val[i] = interpolation_f32(
                    pCalib[ilow].hw_btnrC_mdSigma_curve.val[i], pCalib[ihigh].hw_btnrC_mdSigma_curve.val[i], ratio);
    }
    for (i = 0; i < 16; i++) {
        out->calibDyn.hw_btnrC_preSpNrSgm_curve.idx[i] = pCalib[inear].hw_btnrC_preSpNrSgm_curve.idx[i];
        out->calibDyn.hw_btnrC_preSpNrSgm_curve.val[i] = interpolation_f32(
                    pCalib[ilow].hw_btnrC_preSpNrSgm_curve.val[i], pCalib[ihigh].hw_btnrC_preSpNrSgm_curve.val[i], ratio);
    }
...
XCamReturn Abtnr_processing(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams, int iso)
{
    ...
    int delta_isoCIS = abs(isoCIS - pBtnrCtx->pre_isoCIS);
    if(delta_isoCIS > 0) {
        need_recal = true;
    }
...
#endif
#if RKAIQ_HAVE_BAYERTNR_V51
        BtnrSelectParamCIS(pBtnrCtx, btnr_res, isoCIS);
#endif
        outparams->cfg_update = true;
        outparams->en = btnr_attrib->en;
        outparams->bypass = btnr_attrib->bypass;
