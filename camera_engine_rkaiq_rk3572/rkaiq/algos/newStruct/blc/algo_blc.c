...
processing(const RkAiqAlgoCom* inparams, RkAiqAlgoResCom* outparams)
{
    ...
    int isoCIS = inparams->u.proc.isoCIS;
    int delta_isoCIS = abs(isoCIS - pBlcCtx->isoCIS);
    if(delta_isoCIS > ABLC_RECALCULATE_DELTE_ISO) {
        pBlcCtx->isReCal_ = true;
    }
    ...
    if (pBlcCtx->isReCal_) {
...
#ifdef ISP_HW_V351s
        BlcSelectParamCIS(pBlcCtx, outparams->algoRes, isoCIS);
#endif
        ...
        LOGD_ABLC("old iso %d new %d, blc: %d %d %d %d", pBlcCtx->iso, iso,
                  pAblcProcResParams->dyn.obcPreTnr.hw_blcC_obR_offset,
                  pAblcProcResParams->dyn.obcPreTnr.hw_blcC_obGr_offset,
                  pAblcProcResParams->dyn.obcPreTnr.hw_blcC_obGb_offset,
                  pAblcProcResParams->dyn.obcPreTnr.hw_blcC_obB_offset);
#else
        LOGD_ABLC("old iso %d new %d, blc: %d %d %d %d", pBlcCtx->iso, iso,
                  pAblcProcResParams->dyn.obcPreTnr.hw_blcC_obR_val,
                  pAblcProcResParams->dyn.obcPreTnr.hw_blcC_obGr_val,
...
#endif
#ifdef ISP_HW_V351s
    out->dyn.obcPreTnr.hw_blcC_obR_offset = interpolation_i16(
            paut->dyn[ilow].obcPreTnr.hw_blcC_obR_offset, paut->dyn[ihigh].obcPreTnr.hw_blcC_obR_offset, uratio);
    out->dyn.obcPreTnr.hw_blcC_obGr_offset = interpolation_i16(
                paut->dyn[ilow].obcPreTnr.hw_blcC_obGr_offset, paut->dyn[ihigh].obcPreTnr.hw_blcC_obGr_offset, uratio);
    out->dyn.obcPreTnr.hw_blcC_obGb_offset = interpolation_i16(
                paut->dyn[ilow].obcPreTnr.hw_blcC_obGb_offset, paut->dyn[ihigh].obcPreTnr.hw_blcC_obGb_offset, uratio);
    out->dyn.obcPreTnr.hw_blcC_obB_offset = interpolation_i16(
            paut->dyn[ilow].obcPreTnr.hw_blcC_obB_offset, paut->dyn[ihigh].obcPreTnr.hw_blcC_obB_offset, uratio);
#endif
    return XCAM_RETURN_NO_ERROR;
}
...
#ifdef ISP_HW_V351s
XCamReturn
BlcSelectParamCIS(BlcContext_t* pBlcCtx, blc_param_t* out, int isoCIS)
{
    LOG1_ABLC("%s(%d): enter!\n", __FUNCTION__, __LINE__);
    ...
    float ratio = 0.0f;
    uint16_t uratio;
    blc_calib_dyn_t* pCalib = pBlcCtx->blc_attrib->stAuto.calibDyn;

    pre_interp(isoCIS, pBlcCtx->iso_list, 13, &ilow, &ihigh, &ratio);
    uratio = ratio * (1 << RATIO_FIXBIT);

    out->calibDyn.obMean.hw_blcC_obMeanR_val = interpolation_i16(
                                         pCalib[ilow].obMean.hw_blcC_obMeanR_val, pCalib[ihigh].obMean.hw_blcC_obMeanR_val, uratio);
    out->calibDyn.obMean.hw_blcC_obMeanGr_val = interpolation_i16(
                                          pCalib[ilow].obMean.hw_blcC_obMeanGr_val, pCalib[ihigh].obMean.hw_blcC_obMeanGr_val, uratio);
    out->calibDyn.obMean.hw_blcC_obMeanGb_val = interpolation_i16(
                                          pCalib[ilow].obMean.hw_blcC_obMeanGb_val, pCalib[ihigh].obMean.hw_blcC_obMeanGb_val, uratio);
    out->calibDyn.obMean.hw_blcC_obMeanB_val = interpolation_i16(
                                         pCalib[ilow].obMean.hw_blcC_obMeanB_val, pCalib[ihigh].obMean.hw_blcC_obMeanB_val, uratio);
    int16_t obSigmaR_val = interpolation_i16(
                               pCalib[ilow].obSigma.hw_blcC_obSigmaR_val, pCalib[ihigh].obSigma.hw_blcC_obSigmaR_val, uratio);
    int16_t obSigmaGr_val = interpolation_i16(
                                pCalib[ilow].obSigma.hw_blcC_obSigmaGr_val, pCalib[ihigh].obSigma.hw_blcC_obSigmaGr_val, uratio);
    int16_t obSigmaGb_val = interpolation_i16(
                                pCalib[ilow].obSigma.hw_blcC_obSigmaGb_val, pCalib[ihigh].obSigma.hw_blcC_obSigmaGb_val, uratio);
    int16_t obSigmaB_val  = interpolation_i16(
                               pCalib[ilow].obSigma.hw_blcC_obSigmaB_val, pCalib[ihigh].obSigma.hw_blcC_obSigmaB_val, uratio); 
	int16_t max_tmp1 = MAX(obSigmaR_val, obSigmaGr_val);
    int16_t max_tmp2 = MAX(obSigmaGb_val, obSigmaB_val);
	max_tmp1 = MAX(max_tmp1, max_tmp2);
    out->calibDyn.obSigma.hw_blcC_obSigmaR_val = max_tmp1;
	out->calibDyn.obSigma.hw_blcC_obSigmaGr_val = max_tmp1;
	out->calibDyn.obSigma.hw_blcC_obSigmaGb_val = max_tmp1;
	out->calibDyn.obSigma.hw_blcC_obSigmaB_val = max_tmp1;
