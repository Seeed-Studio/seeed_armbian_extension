...
int aiq_getInt(const cJSON* item) {
...
int aiq_getInt_from_ctx(const c_AIQJsonParserCtx* cJsonParse, const char* key, int* value) {
    cJSON *item = aiq_getObjectItem(aiq_getJsonObject(cJsonParse), key);
    if (aiq_cjson_is_null(item) || !aiq_cjson_is_number(item)) {
        return -1;
    }
...
int aiq_getOptionInt_from_ctx(void *ptr, const char* key, int* value) {
    cJSON *item = (cJSON *)ptr;
    while ((item != NULL) && (item->string != NULL) && (strcmp(key, item->string) != 0))
    {
        item = item->next;
...
int aiq_json_get_int(const c_AIQJsonParserCtx* meta, const char* key, int* value) {
...
int aiq_json_get_option_int(void *ptr, const char *key, int* value) {
