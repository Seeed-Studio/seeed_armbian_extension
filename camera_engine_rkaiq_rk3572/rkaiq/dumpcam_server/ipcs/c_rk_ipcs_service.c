...
#else
            // Close the server socket first so that select() returns immediately
            // (EBADF), allowing the thread loop to check mKeepWorking and exit
            // without waiting for the 1-second select() timeout.
