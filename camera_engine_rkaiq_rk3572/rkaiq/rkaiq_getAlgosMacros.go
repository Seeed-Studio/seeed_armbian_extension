...
func rkaiq_getAlgosMacros(macros_map map[string]bool) []string {
    ...
    if (macros_map["RKAIQ_HAVE_AWB"])                { cflags = append(cflags, "-DRKAIQ_HAVE_AWB=1") }
    if (macros_map["RKAIQ_HAVE_GIC"])                { cflags = append(cflags, "-DRKAIQ_HAVE_GIC=1") }
    if (macros_map["RKAIQ_HAVE_ORB"])                { cflags = append(cflags, "-DRKAIQ_HAVE_ORB=1") }
    if (macros_map["RKAIQ_HAVE_EIS"])                { cflags = append(cflags, "-DRKAIQ_HAVE_EIS=1") }
    if (macros_map["RKAIQ_HAVE_CAC"])                { cflags = append(cflags, "-DRKAIQ_HAVE_CAC=1") }
    if (macros_map["RKAIQ_HAVE_YNR"])                { cflags = append(cflags, "-DRKAIQ_HAVE_YNR=1") }
    if (macros_map["RKAIQ_HAVE_CNR"])                { cflags = append(cflags, "-DRKAIQ_HAVE_CNR=1") }
    if (macros_map["RKAIQ_HAVE_SHARP"])              { cflags = append(cflags, "-DRKAIQ_HAVE_SHARP=1") }
    if (macros_map["RKAIQ_HAVE_MFNR"])               { cflags = append(cflags, "-DRKAIQ_HAVE_MFNR=1") }
    if (macros_map["RKAIQ_HAVE_BAYERNR"])            { cflags = append(cflags, "-DRKAIQ_HAVE_BAYERNR=1") }
    if (macros_map["RKAIQ_HAVE_BAYER2DNR"])          { cflags = append(cflags, "-DRKAIQ_HAVE_BAYER2DNR=1") }
    if (macros_map["RKAIQ_HAVE_BAYERTNR"])           { cflags = append(cflags, "-DRKAIQ_HAVE_BAYERTNR=1") }
    if (macros_map["RKAIQ_HAVE_GAIN"])               { cflags = append(cflags, "-DRKAIQ_HAVE_GAIN=1") }
    if (macros_map["RKAIQ_HAVE_BLC"])                { cflags = append(cflags, "-DRKAIQ_HAVE_BLC=1") }
    if (macros_map["RKAIQ_HAVE_ANR"])                { cflags = append(cflags, "-DRKAIQ_HAVE_ANR=1") }
    if (macros_map["RKAIQ_HAVE_AMD"])                { cflags = append(cflags, "-DRKAIQ_HAVE_AMD=1") }
    if (macros_map["RKAIQ_HAVE_AMTD"])               { cflags = append(cflags, "-DRKAIQ_HAVE_AMTD=1") }
    if (macros_map["RKAIQ_HAVE_AF"])                 { cflags = append(cflags, "-DRKAIQ_HAVE_AF=1") }
    if (macros_map["RKAIQ_HAVE_3DLUT"])              { cflags = append(cflags, "-DRKAIQ_HAVE_3DLUT=1") }
    if (macros_map["RKAIQ_HAVE_CCM"])                { cflags = append(cflags, "-DRKAIQ_HAVE_CCM=1") }
    if (macros_map["RKAIQ_HAVE_GAMMA"])              { cflags = append(cflags, "-DRKAIQ_HAVE_GAMMA=1") }
    if (macros_map["RKAIQ_HAVE_MERGE"])              { cflags = append(cflags, "-DRKAIQ_HAVE_MERGE=1") }
    if (macros_map["RKAIQ_HAVE_DRC"])                { cflags = append(cflags, "-DRKAIQ_HAVE_DRC=1") }
    if (macros_map["RKAIQ_HAVE_DEHAZE"])             { cflags = append(cflags, "-DRKAIQ_HAVE_DEHAZE=1") }
    if (macros_map["RKAIQ_HAVE_TMO"])                { cflags = append(cflags, "-DRKAIQ_HAVE_TMO=1") }
    if (macros_map["RKAIQ_HAVE_WDR"])                { cflags = append(cflags, "-DRKAIQ_HAVE_WDR=1") }
    if (macros_map["RKAIQ_HAVE_LDCH"])               { cflags = append(cflags, "-DRKAIQ_HAVE_LDCH=1") }
    if (macros_map["RKAIQ_HAVE_LDCH_V22"])           { cflags = append(cflags, "-DRKAIQ_HAVE_LDCH_V22=1") }
    if (macros_map["RKAIQ_HAVE_LDCV_V22"])           { cflags = append(cflags, "-DRKAIQ_HAVE_LDCV=1") }
    if (macros_map["RKAIQ_HAVE_FEC"])                { cflags = append(cflags, "-DRKAIQ_HAVE_FEC=1") }
    if (macros_map["RKAIQ_HAVE_CSM"])                { cflags = append(cflags, "-DRKAIQ_HAVE_CSM=1") }
    if (macros_map["RKAIQ_HAVE_CGC"])                { cflags = append(cflags, "-DRKAIQ_HAVE_CGC=1") }
    if (macros_map["RKAIQ_HAVE_AE"])                 { cflags = append(cflags, "-DRKAIQ_HAVE_AE=1") }
    if (macros_map["RKAIQ_HAVE_AFD"])                { cflags = append(cflags, "-DRKAIQ_HAVE_AFD=1") }
    if (macros_map["RKAIQ_HAVE_DEBAYER"])            { cflags = append(cflags, "-DRKAIQ_HAVE_DEBAYER=1") }
    if (macros_map["RKAIQ_HAVE_LSC"])                { cflags = append(cflags, "-DRKAIQ_HAVE_LSC=1") }
    if (macros_map["RKAIQ_HAVE_DPCC"])               { cflags = append(cflags, "-DRKAIQ_HAVE_DPCC=1") }
    if (macros_map["RKAIQ_HAVE_ACP"])                { cflags = append(cflags, "-DRKAIQ_HAVE_ACP=1") }
    if (macros_map["RKAIQ_HAVE_AIE"])                { cflags = append(cflags, "-DRKAIQ_HAVE_AIE=1") }
    if (macros_map["RKAIQ_HAVE_ASD"])                { cflags = append(cflags, "-DRKAIQ_HAVE_ASD=1") }
    if (macros_map["RKAIQ_HAVE_RGBIR_REMOSAIC"])     { cflags = append(cflags, "-DRKAIQ_HAVE_RGBIR_REMOSAIC=1") }
    if (macros_map["RKAIQ_HAVE_RGBIR_REMOSAIC_V10"]) { cflags = append(cflags, "-DRKAIQ_HAVE_RGBIR_REMOSAIC_V10=1") }
    if (macros_map["RKAIQ_HAVE_YUVME"])              { cflags = append(cflags, "-DRKAIQ_HAVE_YUVME=1") }
    if (macros_map["RKAIQ_HAVE_HISTEQ"])             { cflags = append(cflags, "-DRKAIQ_HAVE_HISTEQ=1") }
    if (macros_map["RKAIQ_HAVE_ENHANCE"])            { cflags = append(cflags, "-DRKAIQ_HAVE_ENHANCE=1") }
    if (macros_map["RKAIQ_HAVE_HSV"])                { cflags = append(cflags, "-DRKAIQ_HAVE_HSV=1") }
    ...
    flag0 = macros_map["RKAIQ_HAVE_AWB_V33"]
    if (flag0) {
        cflags = append(cflags, "-DRKAIQ_HAVE_AWB_V33=1")
    }
    ...
    if (flag0) {
        ...
        if (macros_map["RKAIQ_HAVE_CAC_V12"]) {
    ...
    if (macros_map["RKAIQ_HAVE_CNR_V30_LITE"]) {
    ...
    if (flag0) {
        ...
        cflags = append(cflags, "-DRKAIQ_HAVE_LDC=1")
    }
    ...
    if (macros_map["RKAIQ_HAVE_YUVME_V1"]) { cflags = append(cflags, "-DRKAIQ_HAVE_YUVME_V1=1") }
    if (macros_map["RKAIQ_HAVE_CNR_V31"]) { cflags = append(cflags, "-DRKAIQ_HAVE_CNR_V31=1") }
    if (macros_map["RKAIQ_HAVE_BAYERTNR_V41"]) { cflags = append(cflags, "-DRKAIQ_HAVE_BAYERTNR_V41=1") }
    if (macros_map["RKAIQ_HAVE_BAYERTNR_V42"]) { cflags = append(cflags, "-DRKAIQ_HAVE_BAYERTNR_V42=1") }
    if (macros_map["RKAIQ_HAVE_BAYERTNR_V51"]) { cflags = append(cflags, "-DRKAIQ_HAVE_BAYERTNR_V51=1") }
    if (macros_map["RKAIQ_HAVE_YNR_V40"]) { cflags = append(cflags, "-DRKAIQ_HAVE_YNR_V40=1") }
    if (macros_map["RKAIQ_HAVE_YNR_V41"]) { cflags = append(cflags, "-DRKAIQ_HAVE_YNR_V41=1") }
    if (macros_map["RKAIQ_HAVE_SHARP_V40"]) { cflags = append(cflags, "-DRKAIQ_HAVE_SHARP_V40=1") }
    if (macros_map["RKAIQ_HAVE_SHARP_V41"]) { cflags = append(cflags, "-DRKAIQ_HAVE_SHARP_V41=1") }
    if (macros_map["RKAIQ_HAVE_CNR_V35"]) { cflags = append(cflags, "-DRKAIQ_HAVE_CNR_V35=1") }
    if (macros_map["RKAIQ_HAVE_CNR_V36"]) { cflags = append(cflags, "-DRKAIQ_HAVE_CNR_V36=1") }
    if (macros_map["RKAIQ_HAVE_GIC_V3"]) { cflags = append(cflags, "-DRKAIQ_HAVE_GIC_V3=1") }
    if (macros_map["RKAIQ_HAVE_GIC_V40"]) { cflags = append(cflags, "-DRKAIQ_HAVE_GIC_V40=1") }
    if (macros_map["RKAIQ_HAVE_CAC_V30"]) { cflags = append(cflags, "-DRKAIQ_HAVE_CAC_V30=1") }
    if (macros_map["RKAIQ_HAVE_AIBNR"]) { cflags = append(cflags, "-DRKAIQ_HAVE_AIBNR=1") }
    if (macros_map["RKAIQ_HAVE_AIRMS"]) { cflags = append(cflags, "-DRKAIQ_HAVE_AIRMS=1") }
    if (macros_map["RKAIQ_HAVE_AIYNR"]) { cflags = append(cflags, "-DRKAIQ_HAVE_AIYNR=1") }
    if (macros_map["RKAIQ_HAVE_MEMC"]) { cflags = append(cflags, "-DRKAIQ_HAVE_MEMC=1") }
    if (macros_map["RKAIQ_HAVE_FPNSW"]) { cflags = append(cflags, "-DRKAIQ_HAVE_FPNSW=1") }
    if (macros_map["RKAIQ_HAVE_HISTEQ_V10"]) { cflags = append(cflags, "-DRKAIQ_HAVE_HISTEQ_V10=1") }
    if (macros_map["RKAIQ_HAVE_ENHANCE_V10"]) { cflags = append(cflags, "-DRKAIQ_HAVE_ENHANCE_V10=1") }
    if (macros_map["RKAIQ_HAVE_HSV_V10"]) { cflags = append(cflags, "-DRKAIQ_HAVE_HSV_V10=1") }
    if (macros_map["RKAIQ_HAVE_HSV_V11"]) { cflags = append(cflags, "-DRKAIQ_HAVE_HSV_V11=1") }
    if (macros_map["RKAIQ_HAVE_ASD_V11"]) { cflags = append(cflags, "-DRKAIQ_HAVE_ASD_V11=1") }
    ...
    if !flag0 {
        if flag3 {
            macros_map["RKAIQ_HAVE_LDC"] = true
            cflags = append(cflags, "-DRKAIQ_HAVE_LDC=1")
        } else if flag1 || flag2 {
