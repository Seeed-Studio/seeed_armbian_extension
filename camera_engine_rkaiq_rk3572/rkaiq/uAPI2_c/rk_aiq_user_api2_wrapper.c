...
XCamReturn
rk_aiq_uapi2_query_algoInterpIsoInfo(const rk_aiq_sys_ctx_t *aiq_ctx,
                       algo_interp_iso_list_t* info)
{
    ...
    const rk_aiq_sys_ctx_t* sys_ctx = rk_aiq_user_api2_common_getSysCtx(aiq_ctx);
    ...
    for (; i < RESULT_TYPE_MAX_PARAM; i++) {
        ...
        if (algo_handle && algo_handle->queryInterpIso) {
            algo_handle->queryInterpIso(algo_handle,
                &info->isoInfo[i].interpIso, &info->isoInfo[i].isoH, &info->isoInfo[i].isoL, 0);
            algo_handle->queryInterpIso(algo_handle,
                &info->isoCISInfo[i].interpIso, &info->isoCISInfo[i].isoH, &info->isoCISInfo[i].isoL, 1);
        }
        info->isoInfo[i].type = i;
        info->isoCISInfo[i].type = i;
        LOGD("resT:0x%x, interp:%d, isoL:%d, isoH:%d", i, info->isoInfo[i].interpIso, info->isoInfo[i].isoL, info->isoInfo[i].isoH);
        LOGD("resT:0x%x, interp:%d, isoL:%d, isoH:%d", i, info->isoCISInfo[i].interpIso, info->isoCISInfo[i].isoL, info->isoCISInfo[i].isoH);
    }
...
#if USE_NEWSTRUCT
XCamReturn rk_aiq_user_api2_btnr_QueryStats(const rk_aiq_sys_ctx_t* aiq_ctx, btnr_stats_cp_t* stats)
{
    const rk_aiq_sys_ctx_t* sys_ctx = rk_aiq_user_api2_common_getSysCtx(aiq_ctx);
