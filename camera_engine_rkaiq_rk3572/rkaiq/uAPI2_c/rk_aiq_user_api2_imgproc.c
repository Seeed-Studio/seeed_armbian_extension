...
#include "rk_aiq_user_api2_isp351s.h"
#endif
#endif
...
XCamReturn rk_aiq_uapi2_setGammaCoef(const rk_aiq_sys_ctx_t* ctx, float GammaCoef, float SlopeAtZero)
{
    ...
    ret = rk_aiq_user_api2_gamma_GetAttrib(ctx, &gammaAttr);
    RKAIQ_IMGPROC_CHECK_RET(ret, "get gamma attr failed!\nsetGammaCoef failed!");
...
XCamReturn rk_aiq_uapi2_setMirrorFlip(const rk_aiq_sys_ctx_t* ctx, bool mirror, bool flip,
    ...
    btnr_api_attrib_t btnr_attr;
    memset(&btnr_attr, 0, sizeof(btnr_api_attrib_t));
    memset(&btnr_sta, 0, sizeof(btnr_status_t));
    ret = rk_aiq_user_api2_btnr_GetAttrib(ctx, &btnr_attr);
    if (btnr_attr.en && !btnr_attr.bypass) {
...
XCamReturn rk_aiq_uapi2_setColorSpace(const rk_aiq_sys_ctx_t* ctx, int Cspace)
{
    ...
    if (csm_attrib.en == false) {
        LOGK("%s: force enable csm!\n", __FUNCTION__);
        csm_attrib.en = true;
    }

    if (cgc_attrib.en == false) {
        LOGK("%s: force enable cgc!\n", __FUNCTION__);
        cgc_attrib.en = true;
    }
