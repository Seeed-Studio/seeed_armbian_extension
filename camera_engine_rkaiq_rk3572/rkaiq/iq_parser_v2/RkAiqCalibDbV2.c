...
#include <sys/stat.h>
#if defined(__GLIBC__) && defined(__GNU_LIBRARY__) && __GLIBC_MINOR__ >= 14
#include <malloc.h>
#endif
...
CamCalibDbV2Context_t* calibdbV2_ctx_new() {
    ...
    calib_scene = aiq_mallocz(sizeof(CamCalibDbV2ContextIsp351s_t));
#else
...
int CamCalibDbProjFree(CamCalibDbProj_t *calibproj) {
    ...
    j2s_ctx ctx;
    j2s_init(&ctx);
    j2s_struct_free(&ctx, "CamCalibDbProj_t", calibproj);
    j2s_deinit(&ctx);
...
int CamCalibDbReleaseOneCalibDbProj(CamCalibDbProj_t *calibproj) {
    if (!calibproj)
        return 0;

    // Remove from gCalibDbsMap first to avoid dangling pointer
    bool found = false;
    bool is_bin = false;
    aiqMutex_lock(&gCamCalibDbMutex);
    for (int i = 0; i < gCalibDbsMapCnt; i++) {
        if (gCalibDbsMap[i].calibproj == calibproj) {
            is_bin = gCalibDbsMap[i].is_bin;
            gCalibDbsMap[i] = gCalibDbsMap[gCalibDbsMapCnt - 1];
            gCalibDbsMapCnt--;
            found = true;
            break;
        }
...
static void _print_product_info(CalibDbProductInfo_t* pProductInfo)
{
   LOGK("IQ product info:ChipName:%s,Author:%s,IQVer:%s,IQStructMark:%s(Exp:%s),date:%s",
        pProductInfo->chip_name,
        pProductInfo->author,
        pProductInfo->iq_ver,
        pProductInfo->iq_struct_mark, IQ_STRUCT_MARK,
        pProductInfo->date
        );
   return;
}
...
CamCalibDbProj_t *CamCalibDbJson2calibprojFromFile(const char *jsfile) {
    ...
    aiq_free(json_buff);
    _print_product_info(&calibproj->product_info);
...
CamCalibDbProj_t *CamCalibDbCreateCalibDbProjFromFile(int sns_id, const char *jsfile) {
    ...
    if (find_idx != -1) {
        ...
        if (calibproj) {
            ...
            gCalibDbsMap[gCalibDbsMapCnt].is_bin = (0 != access(jsfile, F_OK));
            gCalibDbsMapCnt++;
