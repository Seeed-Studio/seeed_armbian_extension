...
CamCalibDbV2Context_t* RkAiqSceneManagerCreateSceneCalib(
  ...
  int ret = -1;
    ...
    CamCalibDbV2ContextIsp351s_t *calib_scene = aiq_mallocz(sizeof(CamCalibDbV2ContextIsp351s_t));
#else
...
#elif defined(ISP_HW_V351s)
    calib_json = RkCam_cJSONUtils_GetPointer(scene_json, "scene_isp351s");
#else
#error "WRONG ISP_HW_VERSION, ONLY SUPPORT V20 AND V21 AND V30 NOW !"
#endif
