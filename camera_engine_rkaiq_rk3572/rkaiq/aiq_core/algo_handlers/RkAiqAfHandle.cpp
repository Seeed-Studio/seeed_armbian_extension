...
XCamReturn RkAiqAfHandleInt::setOtpAf(struct rkmodule_af_inf *otp_af, struct rkmodule_pdaf_inf *otp_pdaf)
{
    ENTER_ANALYZER_FUNCTION();
...
#if RKAIQ_HAVE_PDAF
    af_proc_res_int->pdaf_type1_param = &shared->fullParams->mPdType1Params->data()->result;
#else
...
XCamReturn RkAiqAfHandleInt::genIspResult(RkAiqFullParams* params, RkAiqFullParams* cur_params) {
...
#if RKAIQ_HAVE_PDAF
    SmartPtr<rk_aiq_pdtype1_params_wrapper_t> pdtype1_param = params->mPdType1Params->data();
#endif
    ...
    if (af_com->pdaf_type1_update) {
        mAPdafType1ResSyncFalg = shared->frameId;
        pdtype1_param->sync_flag = mAPdafType1ResSyncFalg;
        cur_params->mPdType1Params = params->mPdType1Params;
        pdtype1_param->is_update = true;
        af_com->af_focus_update = false;
        LOGD_AF("[%d] pdtype1 params from algo", mAPdafType1ResSyncFalg);
    } else if (mAPdafType1ResSyncFalg != pdtype1_param->sync_flag) {
        pdtype1_param->sync_flag = mAPdafType1ResSyncFalg;
        if (cur_params->mPdType1Params.ptr()) {
            pdtype1_param->result = cur_params->mPdType1Params->data()->result;
            pdtype1_param->is_update = true;
        } else {
            LOGE_AF("no pdtype1 latest params !");
            pdtype1_param->is_update = false;
        }
        LOGD_AF("[%d] pdtype1 params from latest [%d]", shared->frameId, mAPdafType1ResSyncFalg);
    } else {
        // do nothing, result in buf needn't update
        pdtype1_param->is_update = false;
        //LOGD_AF("[%d] pdtype1s params needn't update", shared->frameId);
    }
#endif
