...
typedef struct aiq_isp_effect_params_s {
    aiq_ref_base_t _ref_base;
#if defined(RKAIQ_HAVE_MULTIISP)
    struct isp351s_isp_params_cfg isp_params_v35[2];
#endif
    bool lsc_en;
    struct isp3x_lsc_cfg lsc_cfg;
...
static const char* Cam3aResultType2Str[RESULT_TYPE_MAX_PARAM] = {
    ...
    [RESULT_TYPE_PDAF_TYPE1_PARAM]    = "PDAFTYPE1",
};
...
typedef enum _RkAiqIspUniteMode {
    ...
    RK_AIQ_ISP_UNITE_MODE_1X2 = 1,
    RK_AIQ_ISP_UNITE_MODE_TWO_GRID = 1,
    RK_AIQ_ISP_UNITE_MODE_2X2 = 2,
    RK_AIQ_ISP_UNITE_MODE_FOUR_GRID = 2,
    RK_AIQ_ISP_UNITE_MODE_3X3,
} RkAiqIspUniteMode;
...
typedef struct _RkAiqIspUniteInfo_s {
    RkAiqIspUniteMode mode;
    uint8_t h_num;
    uint8_t v_num;
    Rectangle_t isp_rect[3][3];
} RkAiqIspUniteInfo_t;
...
/**
 * @brief Global 3a Info block type enumeration
 *
 * Identifies the type of 3a data block being passed.
 *
 * @AIQ_G3a_BLOCK_TYPE_AWB_STATS: AWB statistics
 * @AIQ_G3a_BLOCK_TYPE_AWB_RESULT: AWB algorithm result
 * @AIQ_G3a_BLOCK_TYPE_MAX: Maximum block type value
 */
typedef enum AiqGlobal3aBlockType_e {
    AIQ_G3A_BLOCK_TYPE_AWB_STATS = 0,
    AIQ_G3A_BLOCK_TYPE_AWB_RESULT,
    AIQ_G3A_BLOCK_TYPE_MAX
} AiqGlobal3aBlockType_t;

/** Bit mask for valid data types */
#define AIQ_G3A_VALID_AWB_STATS  (1U << AIQ_G3A_BLOCK_TYPE_AWB_STATS)
#define AIQ_G3A_VALID_AWB_RESULT (1U << AIQ_G3A_BLOCK_TYPE_AWB_RESULT)

/**
 * @brief Global 3a block header
 *
 * Common header for all 3a data blocks. Each block type embeds this header
 * as its first member, allowing generic handling of different block types.
 *
 * For example:
 *
 * .. code-block:: c
 *
 *      void populate_awb_stats(AiqGlobal3aBlockHeader_t *block,
 *                              const aiq_awb_stats_wrapper_t *stats) {
 *              AiqGlobal3aBlockAwbStats_t *awb =
 *                      (AiqGlobal3aBlockAwbStats_t *)block;
 *
 *              awb->header.type = AIQ_G3a_BLOCK_TYPE_AWB_STATS;
 *              awb->header.flags = 0;
 *              awb->header.size = sizeof(*awb);
 *
 *              awb->config = *stats;
 *      }
 *
 * @type: The block type, see AiqGlobal3aBlockType_t
 * @flags: Reserved for future use (e.g., enable/disable flags)
 * @size: Size (in bytes) of the entire block, including this header
 */
typedef struct AiqGlobal3aBlockHeader_s {
    uint16_t type;
    uint16_t flags;
    uint32_t size;
} AiqGlobal3aBlockHeader_t;

/**
 * @brief AWB statistics block
 *
 * @header: Block header
 * @config: AWB statistics data
 */
typedef struct AiqGlobal3aBlockAwbStats_s {
    AiqGlobal3aBlockHeader_t header;
    aiq_awb_stats_wrapper_t* config;
} AiqGlobal3aBlockAwbStats_t;

/**
 * @brief AWB algorithm result data
 *
 * @frame_id: Frame ID
 * @awbConverged: AWB convergence status
 * @cctGloabl: Global color temperature
 * @LVValue: Light value
 * @awb_gain_algo: AWB gain from algorithm
 * @stat_gain_glb: Global statistics gain
 * @stat_gain_blk: Block statistics gain
 */
typedef struct AiqAwbAlgoResultData_s {
    uint32_t frame_id;
    bool awbConverged;
    rk_aiq_wb_cct_t cctGloabl;
    float    fLVValue;//Y
    float    fRGBLv[3];//R,G,B
    rk_aiq_wb_gain_t awb_gain_algo;
    rk_aiq_wb_gain_t awb_gain_befmulcamfuse;
    //rk_aiq_wb_gain_t stat_gain_blk;
    rk_aiq_awb_stat_res_full2_t stat_res;
} AiqAwbAlgoResultData_t;

/**
 * @brief AWB algorithm result block
 *
 * @header: Block header
 * @config: Pointer to AWB algorithm result data
 */
typedef struct AiqGlobal3aBlockAwbResult_s {
    AiqGlobal3aBlockHeader_t header;
    RkAiqAlgoProcResAwb* config;
} AiqGlobal3aBlockAwbResult_t;

/**
 * @brief Union of all 3a block types
 *
 * This union allows treating any 3a block generically via the header,
 * while still providing access to the specific block type when needed.
 *
 * @header: Common header for type identification
 * @awb_stats: AWB statistics block
 * @awb_result: AWB result block
 */
typedef union AiqGlobal3aBlockConfig_u {
    AiqGlobal3aBlockHeader_t header;
    AiqGlobal3aBlockAwbStats_t awb_stats;
    AiqGlobal3aBlockAwbResult_t awb_result;
} AiqGlobal3aBlockConfig_t;

/**
 * @brief Global 3a Info structure for one frame
 *
 * Contains all types of 3a data for a single frame.
 *
 * @_ref_base: Reference counting base
 * @cam_id: Camera ID
 * @frame_id: Frame ID
 * @validMask: Bitmask of valid data types
 * @awb_stats: AWB statistics
 * @awb_result: AWB algorithm result data
 */
typedef struct AiqGlobal3AInfo_s {
    aiq_ref_base_t _ref_base;
    uint32_t cam_id;
    uint32_t frame_id;
    uint32_t validMask;
    aiq_awb_stats_wrapper_t awb_stats;
