        ...
        uint32_t sns_v4l_pix_fmt, int8_t sns_bpp)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if ( (((width / 2 - RKMOUDLE_UNITE_EXTEND_PIXEL) *  sns_bpp / 8) & 0xf) || _is_split) {
            ...
            int mem_mode = CSI_LVDS_MEM_COMPACT;
            int ret1 = _dev[i]->io_control (RKCIF_CMD_SET_CSI_MEMORY_MODE, &mem_mode);
            if (ret1) {
                LOGE_CAMHW_SUBM(ISP20HW_SUBM, "set CSI_LVDS_MEM_COMPACT failed!\n");
                ret = XCAM_RETURN_ERROR_IOCTL;
            } else {
                LOGD_CAMHW_SUBM(ISP20HW_SUBM,"set the memory mode of vicap to big align");
            }
...
XCamReturn
RawStreamCapUnit::set_csi_mem_word_align_mode(int mode)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (ret1) {
            LOGE_CAMHW_SUBM(ISP20HW_SUBM, "device(%s) set memory mode %d failed!",
                            XCAM_STR(_dev[i]->get_device_name()), mode);
            ret = XCAM_RETURN_ERROR_IOCTL;
        } else {
            LOGD_CAMHW_SUBM(ISP20HW_SUBM, "set the memory mode of vicap(%s) to %d",
                            XCAM_STR(_dev[i]->get_device_name()), mode);
        }
    }
