...
#define RWNX_FW_TRACE_ENTRY_SIZE(a)                                            \
	(RWNX_FW_TRACE_NB_PARAM(a) + RWNX_FW_TRACE_HEADER_LEN)
...
#define RWNX_FW_TRACE_RESET_SIZE                                               \
	sizeof(RWNX_FW_TRACE_RESET) - 1 // don't count '\0'
...
static void rwnx_fw_trace_work(struct work_struct *ws)
{
	...
	struct rwnx_fw_trace *trace =
		container_of(dw, struct rwnx_fw_trace, work);

	if (trace->closing || (!rwnx_fw_trace_empty(&trace->buf) &&
	...
	if (waitqueue_active(&trace->queue) && !delayed_work_pending(dw)) {
		schedule_delayed_work(
			dw, msecs_to_jiffies(RWNX_FW_TRACE_CHECK_INT_MS));
	}
}
#endif
			   ...
			   struct rwnx_fw_trace_ipc_desc *ipc)
{
	...
	shared_buf->data =
		(uint16_t *)((uint8_t *)(&ipc->offset) + ipc->offset);
	...
	if ((ipc->nb_compo >> 16) == RWNX_FW_TRACE_READY) {
		...
		shared_buf->compo_table =
			(uint32_t *)((uint8_t *)(&ipc->offset_compo) +
				     ipc->offset_compo);
	} else {
...
static uint16_t *rwnx_fw_trace_to_str(uint16_t *trace, char *buf, size_t *size)
{
	...
	while (nb_param > 0) {
		res = scnprintf(&buf[buf_idx], left, RWNX_FW_TRACE_PARAM_FMT,
				*trace++);
		buf_idx += res;
		left -= res;
		nb_param--;
			      ...
			      struct rwnx_fw_trace_local_buf *local_buf)
{
	...
	while (1) {
		...
		if (local_buf->free_space >= entry_size) {
			...
			if ((local_buf->write < local_buf->read) ||
			    contiguous >= entry_size) {
				/* enough contiguous memory from local_buf->write */
				rwnx_fw_trace_copy_entry(local_buf, ptr,
							 entry_size);
				ret++;
			} else if ((local_buf->free_space - contiguous) >=
				   entry_size) {
 ...
 */
static size_t
rwnx_fw_trace_read_local(struct rwnx_fw_trace_local_buf *local_buf,
			 char __user *user_buf, size_t size)
{
	uint16_t *ptr;
	char str[512]; // worst case 255 params
	size_t str_size;
	int entry_size;
	...
	while (local_buf->nb_entries && !not_cpy) {
		if (local_buf->show_reset == ptr) {
			...
			local_buf->show_reset = NULL;
			not_cpy = copy_to_user(user_buf + res,
					       RWNX_FW_TRACE_RESET,
			  ...
			  bool dont_wait, char __user *user_buf, size_t size)
{
	...
	while (1) {
		size_t read;
		read = rwnx_fw_trace_read_local(local_buf, user_buf + res,
						size - res);
		res += read;
		rwnx_fw_trace_copy(trace, local_buf);
		if (!read)
...
void _rwnx_fw_trace_dump(struct rwnx_fw_trace_buf *trace_buf)
{
	...
	char buf[512]; // worst case 255 params
	size_t size;
 ...
 */
static uint32_t
rwnx_fw_trace_get_trace_level(struct rwnx_fw_trace_buf *trace_buf,
	...
	while (token) {
		...
		if ((sscanf(token, "%d%c0x%x", &compo, &action, &id) == 3) ||
			...
			if (action == '=') {
				...
				ret = rwnx_fw_trace_get_trace_level(trace_buf,
								    compo);
				if (action == '+')
					level = (ret | id);
				else if (action == '-')
...
int rwnx_fw_trace_save_filters(struct rwnx_fw_trace *trace)
{
	...
	saved_filters =
		kmalloc(saved_filters_cnt * sizeof(uint32_t), GFP_KERNEL);
	if (!saved_filters)
		return -1;

	for (i = 0; i < saved_filters_cnt; i++) {
		saved_filters[i] =
			rwnx_fw_trace_get_trace_level(&trace->buf, i);
	}
