			    ...
			    struct rx_vec_detail_2 *rx_vect2)
{
	...
	if (machw_type == RWNX_MACHW_HE) {
		rx_vect1->rssi_leg = rx_vect1->rssi_leg; /* FIXME: */
		return;
	}
	...
	// Reset it
	memset(rx_vect1, 0, sizeof(struct rx_vec_detail_1));
	...
	rx_vect1->rssi_leg = rx_vect1_nx.rssi1;
	...
	case FORMATMOD_VHT:
		...
		rx_vect1->vht.nss = rx_vect1_nx.stbc ? rx_vect1_nx.n_sts / 2 :
						       rx_vect1_nx.n_sts;
		rx_vect1->vht.fec = rx_vect1_nx.fec_coding;
		rx_vect1->vht.length =
			(rx_vect1_nx._ht_length << 16) | rx_vect1_nx.ht_length;
		break;
	}
	...
	// Reset it
	memset(rx_vect2, 0, sizeof(struct rx_vec_detail_2));
