...
const char *ftrace_print_hwq(struct trace_seq *p, int hwq_idx)
{
	static const struct trace_print_flags symbols[] = {
		{ RWNX_HWQ_BK, "BK" },	   { RWNX_HWQ_BE, "BE" },
		{ RWNX_HWQ_VI, "VI" },	   { RWNX_HWQ_VO, "VO" },
		{ RWNX_HWQ_BCMC, "BCMC" }, { -1, NULL }
	};
...
TRACE_EVENT(txq_drop_skb,
	    ...
	    TP_printk("%s skb=%p time_queued=%dms queue_len=%d (retry = %d)",
		      ...
		      jiffies_to_msecs(__entry->queued_time), __entry->q_len,
		      __entry->q_len_retry));
...
TRACE_EVENT(process_hw_queue, TP_PROTO(struct rwnx_hwq *hwq), TP_ARGS(hwq),
	    ...
	    TP_fast_assign(int i; __entry->hwq = hwq->id;
...
TRACE_EVENT(
	...
	TP_fast_assign(int i; __entry->skb = skb; __entry->txq_idx = txq->idx;
	...
	TP_printk("%s skb=%p hw_queue=%s, hw_credits=%s, txq_credits=%d (+%d)"
		  ...
		  __entry->sw_credit_up, __entry->sn, __entry->ampdu_size
#ifdef CONFIG_RWNX_SPLIT_TX_BUF
		  ,
		  __entry->amsdu
#endif
...
DEFINE_EVENT(ps_template, ps_drop, TP_PROTO(struct rwnx_sta *sta),
	     TP_ARGS(sta));
...
TRACE_EVENT(amsdu_len_update, TP_PROTO(struct rwnx_sta *sta, int amsdu_len),
	    TP_ARGS(sta, amsdu_len),
	    TP_STRUCT__entry(__field(u8, sta_idx) __field(u16, amsdu_len)),
	    TP_fast_assign(__entry->sta_idx = sta->sta_idx;
			   __entry->amsdu_len = amsdu_len;),

	    TP_printk("[Sta %d] A-MSDU len = %d", __entry->sta_idx,
		      __entry->amsdu_len));
#endif
...
DECLARE_EVENT_CLASS(ke_msg_template, TP_PROTO(u16 id), TP_ARGS(id),
...
DEFINE_EVENT(ke_msg_template, msg_send, TP_PROTO(u16 id), TP_ARGS(id));

DEFINE_EVENT(ke_msg_template, msg_recv, TP_PROTO(u16 id), TP_ARGS(id));
