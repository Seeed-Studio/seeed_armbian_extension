					  ...
					  size_t extra_ies_len)
{
	...
	if (!sband) {
		WQ_DBG(DM_IEEE80211, DL_ERR, "%s: sband == NULL\n", __func__);
		rcu_read_unlock();
		return;
	}
					...
					size_t extra_ies_len)
{
	...
	memcpy(&ht_cap, &sband->ht_cap, sizeof(ht_cap));
	if (ht_cap.ht_supported && !ap_sta->ht_cap_info && sta->ht_cap_info) {
	...
	memcpy(&vht_cap, &sband->vht_cap, sizeof(vht_cap));
	if (vht_cap.vht_supported && !ap_sta->vht_cap_info &&
	    sta->vht_cap_info) {
				    ...
				    struct cfg80211_chan_def *chandef)
{
	...
	skb = netdev_alloc_skb(
		...
		sizeof(struct ieee80211_tdls_data) + // ethhdr + TDLS info
			...
			(2 + rwnx_band_2GHz->n_channels
#ifdef SUPPORT_5G_BAND
			 + rwnx_band_5GHz->n_channels
#endif
			 ) + /* supported channels */
	...
	rwnx_start_xmit(skb, rwnx_vif->ndev);
