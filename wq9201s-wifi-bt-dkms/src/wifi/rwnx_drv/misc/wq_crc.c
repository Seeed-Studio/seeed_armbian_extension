...
/*static*/ const static uint32_t crc32_table[256] = {
 ...
 */
const static uint32_t crc24_table[256] = {
...
const static uint16_t crc16_table_ccitt[256] = {
...
/* CRC16 implementation acording to CCITT standards from linux kernel */
const static uint16_t crc_ccitt_table[256] = {
	0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48,
	0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7, 0x1081, 0x0108,
	0x3393, 0x221a, 0x56a5, 0x472c, 0x75b7, 0x643e, 0x9cc9, 0x8d40, 0xbfdb,
	0xae52, 0xdaed, 0xcb64, 0xf9ff, 0xe876, 0x2102, 0x308b, 0x0210, 0x1399,
...
{
	uint32_t crc = getcrc24_update(0x00FFFFFF, buffer, len);
	return crc;
}
			       ...
			       uint32_t len)
{
	...
	for (counter = 0; counter < len; counter++)
		crc = (crc << 8) ^
		      crc16_table_ccitt[((crc >> 8) ^ *buffer++) & 0x00FF];
	return crc;
}
