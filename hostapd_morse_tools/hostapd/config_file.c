...
struct hostapd_config * hostapd_config_read(const char *fname)
{
		...
		if (!in_raw && (os_strcmp(buf, "raw") == 0) && (os_strchr(pos, '{') != 0)) {
			...
			if (raw_num >= 0) {
				...
				if (os_strstr(buf, "enabled") != 0) {
					...
					raw->nominal_stas_per_beacon = temp;
				} else {
					errors++;
				}
	...
	morse_cli(conf->last_bss->iface, "raw disable");
	for (ii = 0; ii < MORSE_MAX_RAWS; ii++) {
		int ret = 0;
		struct raw_conf *raw = &conf->last_bss->raw[ii];

		wpa_printf(MSG_INFO, "RAW Settings: %s %u %u %u %s",
			   raw->enabled ? "enable" : "disable",
			   raw->start_time_us,
			   raw->duration_us,
			   raw->slots,
			   raw->cross_slot ? "enable" : "disable",
			   raw->max_beacon_spread,
			   raw->nominal_stas_per_beacon);

		if (raw->enabled) {
			ret = morse_cli(conf->last_bss->iface, "raw enable %u %u %u %u %s %u %u",
				ii,
				raw->start_time_us,
				raw->duration_us,
				raw->slots,
			...
			ret = morse_cli(conf->last_bss->iface, "raw disable %u", ii);
		}

		if (ret)
			wpa_printf(MSG_ERROR, "Unable to set RAW %u on interface %s"
				   "'%s'", ii, &conf->last_bss->iface);
	}

	if (conf->last_bss->raw_enabled) {
		int ret = 0;

		ret = morse_cli(conf->last_bss->iface, "raw enable");
		if (ret)
			wpa_printf(MSG_ERROR, "Unable to enable RAW on interface %s",
				   &conf->last_bss->iface);
	}
#endif
